...
```bash
uv run --script skills/current/vox-interpres/scripts/cli.py ask <audio-file> "tempo and key?" --refresh
```
...
```bash
uv run --script skills/current/vox-interpres/scripts/cli.py analyze <audio-file> --segment-start 60 --segment-duration 30 --json
```
...
```bash
uv run --script skills/current/vox-interpres/scripts/cli.py ask <audio-file> "show metadata codec bitrate sample rate channels" --refresh
```
