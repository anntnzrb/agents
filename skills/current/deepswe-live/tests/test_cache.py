...
def test_store_writes_exact_content_addressed_bytes_and_provenance(
    ...
    assert sidecar["metadata"]["release"] == "v1.1"
    assert sidecar["metadata"]["artifact"] == "leaderboard-live.json"
    assert sidecar["metadata"]["etag"] == '"fixture-etag"'
...
def test_metadata_urls_and_nested_credentials_are_redacted_everywhere(
    ...
    assert (
        sidecar["metadata"]["url"]
        == "https://example.test/data.json?api_key=<redacted>&keep=value"
    )
    assert sidecar["metadata"]["headers"]["Authorization"] == "<redacted>"
    assert sidecar["metadata"]["nested"][0]["token"] == "<redacted>"  # noqa: S105
...
def test_atomic_failure_leaves_no_partial_files(
    ...
    monkeypatch.setattr(cache.Path, "replace", fail_replace)
    store = cache.ArtifactStore(tmp_path)
    digest = hashlib.sha256(BODY).hexdigest()

    with pytest.raises(OSError, match="simulated atomic replace failure"):
        store.store(SOURCE_KEY, BODY, METADATA)
...
def test_raw_and_sidecar_are_immutable_but_source_index_can_move(
    ...
    store.store(SOURCE_KEY, BODY, {**METADATA, "etag": "changed"})
...
def test_legacy_promotion_preserves_caller_file_and_marks_record(
    ...
    assert metadata["metadata"]["legacy_path"] == str(legacy_path)
...
def test_manifest_is_content_addressed_and_immutable(tmp_path: Path) -> None:
    ...
    assert first == second
    manifest_path = tmp_path / first["path"]
