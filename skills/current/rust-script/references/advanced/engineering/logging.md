...
This reference is deliberately stack-agnostic. It never tells you which logging library to use; the destination skill's guidance may suggest defaults, while the project's existing practice overrides everything (Rule 0).
...
4. **The stack's standard structured logger** (use the repository's configured choice, or the destination skill's guidance for greenfield work): never a hand-rolled one
