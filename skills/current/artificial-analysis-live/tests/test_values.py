...
# ruff: noqa: CPY001, INP001, PLR2004, S101
...
def test_compact_json_is_finite_compact_and_deterministic() -> None:  # noqa: D103
    value = {
...
def test_compact_json_rejects_non_finite_numbers(non_finite: object) -> None:  # noqa: D103
    with pytest.raises(ValueError, match=r"non-finite|Out of range"):
        compact_json({"value": non_finite})


def test_source_evidence_to_dict_defends_all_wire_fields() -> None:  # noqa: D103
...
def test_numeric_evidence_keeps_raw_input_and_exact_json_shape() -> None:  # noqa: D103
...
def test_true_zero_is_a_real_eligible_value(raw_value: object) -> None:  # noqa: D103
...
def test_boolean_and_non_finite_values_are_unparsed_and_retained(  # noqa: D103
...
def test_placeholders_are_missing_evidence_with_exact_fields(  # noqa: D103
...
def test_explicit_chart_zero_is_missing_but_literal_zero_is_not(marker: object) -> None:  # noqa: D103
...
def test_chart_zero_placeholder_handles_huge_numeric_input() -> None:  # noqa: D103
...
def test_malformed_and_range_values_remain_unparsed(  # noqa: D103
...
def test_percent_and_ratio_out_of_range_values_are_blocked(  # noqa: D103
    raw_value: object,
...
def test_unit_mismatch_is_retained_as_an_unparsed_raw_value() -> None:  # noqa: D103
...
def test_explicit_percent_and_ratio_normalization(  # noqa: D103, PLR0913, PLR0917
    raw_value: object,
    normalization: str | None,
    unit: str | None,
...
def test_numeric_evidence_rejects_boolean_normalized_values(  # noqa: D103
    normalized_value: bool,  # noqa: FBT001
) -> None:
    with pytest.raises(TypeError, match="must not be bool"):
        NumericEvidence(raw_value=normalized_value, normalized_value=normalized_value)


@pytest.mark.parametrize(
    "normalized_value", [float("nan"), float("inf"), float("-inf")]
)
def test_numeric_evidence_rejects_non_finite_normalized_values(  # noqa: D103
...
def test_numeric_evidence_clears_normalized_values_for_non_normalized_statuses(  # noqa: D103
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {"metric_semantics_status": MetricSemanticsStatus.AMBIGUOUS},
            ("semantics_ambiguous",),
        ),
        (
            {"blocked_reasons": ("source_untrusted", "source_untrusted")},
            ("source_untrusted",),
        ),
    ],
)
def test_status_semantics_and_custom_blockers_are_independent(  # noqa: D103
    kwargs: dict[str, object], expected_reasons: tuple[str, ...]
) -> None:
    evidence = parse_numeric(5, **kwargs)

    expected_normalized = (
        None if kwargs.get("value_status") is ValueStatus.MISSING else 5
    )
...
def test_explicit_blocked_eligibility_has_a_blocker_even_without_other_reasons() -> (  # noqa: D103
    None
):
    evidence = NumericEvidence(
        raw_value=7,
...
def test_comparison_blockers_keep_independent_reasons_and_stable_order() -> None:  # noqa: D103
