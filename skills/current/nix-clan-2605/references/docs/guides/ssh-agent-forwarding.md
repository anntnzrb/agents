...
Only if you set `deploy.buildHost` to something other than `deploy.targetHost`. If they're the same, or `buildHost` is unset, you can skip this guide. If you're new to `buildHost` itself, start with the [Build Host](build-host.md) guide and come back here when it points you to this page.
:::
...
Private Git flake inputs do not require agent forwarding. Clan evaluates your flake on your workstation, so private inputs are fetched locally with your own SSH key and never need to reach the build or target host. See [Private Flake Inputs](private-inputs.md) for the full setup.
:::
