...
An SSH key on your workstation (see [Create an SSH key](../getting-started/create-an-ssh-key.md)) and an account on the Git host that can read the private repository.
:::
...
If you also set `deploy.buildHost`, the build-to-target SSH hop is a separate authentication problem from flake input fetching. See the [Build Host](build-host.md) guide for that one.
