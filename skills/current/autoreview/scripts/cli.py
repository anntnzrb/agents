...
def git_line_ending_args(repo: Path) -> list[str]:
    ...
    if any(
        source.exists()
        and (not source.is_file() or not external_env_path(repo, str(source)))
...
def safe_temp_root(repo: Path, *, engine: str | None = None) -> Path:
    ...
    if (
        ...
        and any(is_within(root, Path(scratch)) for scratch in CODEX_MACOS_SCRATCH_ROOTS)
    ):
        raise SystemExit(
...
def proxy_credential_pattern(values: tuple[str, ...]) -> re.Pattern[str] | None:
    ...
    for value in values:
        ...
        if not parsed.password and user:
            username_forms = {
                parsed.username,
                urllib.parse.quote_from_bytes(user, safe=""),
            }
            try:
    ...
    for form in sorted(forms, key=len, reverse=True):
        ...
        escaped = re.sub(
            r"%[0-9a-fA-F]{2}", lambda match: "(?i:" + match[0] + ")", escaped
        )
        patterns.append(escaped)
    for label, credentials in (
        ("password", labeled_passwords),
...
def redact_proxy_credentials(text: str) -> str:
    pattern = proxy_credential_pattern(
        tuple(os.environ.get(key, "") for key in PROXY_ENV_KEYS)
    )
...
def redact_proxy_report(value: Any) -> Any:
    ...
    if isinstance(value, dict):
        ...
        return {
            key: item if key in enums else redact_proxy_report(item)
...
def safe_engine_env(
    ...
    *,
    ...
    for key in TRANSPORT_TRUST_PATH_ENV_KEYS:
        value = os.environ.get(key)
        normalized = (
            normalize_external_env_path_value(repo, key, value) if value else None
        )
        if normalized:
            env[key] = normalized
    if engine == "codex":
...
def _await_owned_process_groups(
    procs: list[subprocess.Popen], grace_seconds: float
) -> None:
...
class OwnedProcessSignalHandlers:
    ...
    def __enter__(self) -> OwnedProcessSignalHandlers:
        self.previous = {
            signum: signal.getsignal(signum) for signum in _handled_signal_numbers()
        }
...
def choose_target(
    ...
    if mode == "local" or (mode == "auto" and is_dirty(repo)):
        ...
        return "local", validate_git_ref(
            repo, base_ref, "base", pin=True
...
def resolve_command(name: str, repo: Path) -> str:
    ...
    if resolved:
        return resolved
    raise SystemExit(
        f"executable not found: {name}. Install it or pass an explicit trusted path when supported."
    )
...
def find_command(name: str, repo: Path) -> str | None:
    ...
    for part in os.environ.get("PATH", "").split(os.pathsep):
        ...
        if is_within(resolved_part, resolved_repo):
            continue
        found = first_executable_candidate(
            resolved_part / name, reject_root=resolved_repo
        )
        if found:
            return found
    return None
...
def has_directory_component(name: str, command: Path) -> bool:
    ...
    return (
        command.is_absolute()
        or bool(command.drive)
        or any(separator in name for separator in separators)
    )


def first_executable_candidate(
    path: Path, *, reject_root: Path | None = None
) -> str | None:
...
def read_file_bytes(path: Path, limit: int | None = None) -> bytes:
    ...
    try:
        ...
        raise SystemExit(
            f"unreadable file: {display_escape(path, 500)}: {display_escape(exc, 500)}"
        ) from exc
    finally:
...
def sensitive_repo_path_risk(rel: str) -> str | None:
    ...
    credential_directory = any(
        TRACKED_CREDENTIAL_DIR_PATTERN.fullmatch(part) for part in path.parts[:-1]
    )
    if (
...
def credential_store_path(normalized: str) -> bool:
    ...
    return (
        ...
        and credential_data_file
        and not skill_instruction_path(path)
    )
...
def tracked_sensitive_paths(paths: list[str]) -> set[str]:
    return {rel for rel in paths if tracked_sensitive_repo_path_risk(rel) is not None}
...
def require_no_binary_diff(label: str, numstat: str) -> None:
    ...
    if binary_paths:
        details = "\n".join(
            f"- {display_escape(path, 500)}" for path in binary_paths[:20]
        )
...
def require_no_gitlink_diff(label: str, raw_diff: str) -> None:
    ...
    if gitlink_paths:
        details = "\n".join(
            f"- {display_escape(path, 500)}" for path in gitlink_paths[:20]
        )
        more = (
            f"\n... {len(gitlink_paths) - 20} more" if len(gitlink_paths) > 20 else ""
        )
        raise SystemExit(
...
def linked_worktree_boundary(
    ...
    try:
        ...
        return None
    if (
        source_file_fingerprint(git_file) != before
    ...
    return (
        ...
        topology,
        before,
        hashlib.sha256(backlink_bytes).hexdigest(),
    )
...
def untracked_sources(
    ...
    root = repo.resolve()
    common_dir = (
        root / git(repo, "rev-parse", "--git-common-dir").removesuffix("\n")
    ).resolve()
    worktrees_dir = (
        root / git(repo, "rev-parse", "--git-path", "worktrees").removesuffix("\n")
    ).resolve()
    worktrees = []
    ...
    return UntrackedSources(
        [rel for rel in files if rel not in excluded], tuple(sorted(worktrees))
    )
...
def collect_untracked_file_snapshots(
    ...
    for rel in files:
        ...
        if risk:
            if sensitive_repo_path_risk(rel) is not None or risk in {
                "symlink",
...
def git_source_version(repo: Path, path: str, ref: str | None) -> SourceVersion:
    ...
    entries = [
        metadata
        for metadata, actual in (entry.split("\t", 1) for entry in entries)
        if actual == path
        and not (ref is not None and metadata.startswith("040000 tree "))
    ]
    if not entries:
    ...
    if (ref is None and last != "0") or mode not in {"100644", "100755"}:
        raise SystemExit(
            f"unsafe mixed source mode: {display_escape(path, 500)} ({mode})"
        )
    return SourceVersion(f"git:{oid}:{mode}", mode, None)


def read_git_source(
    repo: Path, path: str, version: str, source: SourceVersion
) -> SourceVersion:
    if source.mode is None:
        return source
    oid = source.identity.split(":")[1]
    content = mixed_source_text(
        path, version, git_bytes(repo, "cat-file", "blob", oid).stdout
    )
...
def working_source_version(
    ...
    return SourceVersion(
        f"sha256:{hashlib.sha256(data).hexdigest()}:{mode}", mode, content
    )
...
def local_patch_ownership(patch: str, paths: list[str]) -> list[tuple[str, str]]:
    ...
    units = review_bundle_units(patch)
    if len(units) != len(paths) or any(
        not unit.startswith("diff --git ") for unit in units
    ):
...
def capture_mixed_paths(
    ...
    for path in sorted(paths):
        ...
        base = (
            git_source_version(repo, path, base_ref)
            if base_ref
            else SourceVersion("absent", None, None)
        )
        index = read_git_source(
            repo, path, "index", git_source_version(repo, path, None)
        )
        ...
        for source, removed in ((base, index_removed), (index, working_removed)):
            ...
            if any(
                line < 1
                or line > len(lines)
                or lines[line - 1].removesuffix("\n") != text
                for line, text in removed
            ):
                raise SystemExit(
                    f"mixed deletion source does not match patch: {display_escape(path, 500)}"
                )
...
def verify_mixed_sources(repo: Path, records: tuple[MixedPath, ...]) -> None:
    for record in records:
        if (
            mixed_source_topology(repo, record.path) != record.topology
            or git_source_version(repo, record.path, None).identity
            != record.index.identity
            or working_source_version(repo, record.path) != record.working_tree
        ):
            raise SystemExit(
                "mixed source changed; rerun autoreview against the updated inputs"
            )
...
def local_bundle(repo: Path, base_ref: str | None = None) -> CapturedBundle:
    ...
    staged_patch = git(
        repo, "diff", *SAFE_DIFF_FLAGS, "--cached", "--patch", *staged_base
    )
    unstaged_patch = git(repo, "diff", *SAFE_DIFF_FLAGS, "--patch")
    require_no_binary_diff(
        "local staged diff",
        git(
            repo, "diff", *SAFE_DIFF_FLAGS, "--cached", "--numstat", "-z", *staged_base
        ),
    )
    require_no_binary_diff(
        "local unstaged diff",
    ...
    staged_raw = git(
        repo, "diff", *SAFE_DIFF_FLAGS, "--cached", "--raw", "-z", *staged_base
    )
    ...
    for index in range(0, len(raw_records) - 1, 2):
        ...
        if metadata.startswith(":") and metadata.split()[-1] == "D":
            ...
            if (
                path not in staged_blocked_paths
                and working_blob_stat(repo / path) is not None
    ...
    mixed_paths = (
        (set(staged_paths) & (set(unstaged_paths) | set(untracked)))
        - staged_blocked_paths
        - unstaged_blocked_paths
    )
    staged_units = (
        local_patch_ownership(staged_patch, staged_paths) if mixed_paths else []
    )
    unstaged_units = (
        local_patch_ownership(unstaged_patch, unstaged_paths) if mixed_paths else []
    )
    ...
    for part_index, part in enumerate(parts):
        if part_index in owned_parts and mixed_paths:
            ...
            retained = [
                ...
                if not tracked_sensitive_repo_path_risk(path)
            ]
            prefix = (
    ...
    mixed = (
        capture_mixed_paths(
            ...
            mixed_paths,
            transitions["index"],
            transitions["working_tree"],
            dict(untracked_snapshots),
        )
        if mixed_paths
        else ()
    )
    paths = (set(staged_paths) - staged_blocked_paths) | (
        set(unstaged_paths) - unstaged_blocked_paths
    )
...
def source_file_fingerprint(
    ...
    if stat.S_ISLNK(before.st_mode):
        ...
        if (
            ...
            raise SystemExit(f"file changed while reading: {display_escape(path, 500)}")
    ...
    for rel in sorted(set(tracked + untracked.files)):
        fingerprints.append(
            (
                ...
                if index_modes.get(rel) == "160000" and (repo / rel / ".git").exists()
...
def commit_diff_refs(repo: Path, commit_ref: str) -> tuple[str, ...]:
    ...
    if (
        parents
        and git_result(
            repo, "cat-file", "-e", f"{parents[0]}^{{commit}}", check=False
        ).returncode
    ):
        raise SystemExit(
...
def commit_bundle(repo: Path, commit_ref: str) -> CapturedBundle:
    ...
    commit_paths = git_path_list(
        repo, "diff-tree", *COMMIT_DIFF_FLAGS, "--name-only", "-z", *refs
    )
    require_no_binary_diff(
        "commit diff",
    ...
    parent_header = (
        f"parent: {refs[0]}" if len(refs) == 2 else "parent: none (verified raw root)"
    )
    text = "\n\n".join(
        [
            "# Commit Diff",
...
def relative_evidence_path(repo: Path, raw_path: str, label: str) -> Path:
    ...
    if (
        ...
        and ".." not in original.parts
    ):
        try:
            original = original.relative_to(repo.resolve())
        except ValueError:
            raise SystemExit(
                f"{label} must be inside the reviewed repository: {raw_path}"
            ) from None
...
def evidence_topology(repo: Path, raw_path: str) -> tuple[tuple[int, int, int], ...]:
    ...
    for part in Path(raw_path).parts:
        ...
        try:
            ...
            raise SystemExit(
                f"evidence changed or became unreadable: {raw_path}"
            ) from exc
        if stat.S_ISLNK(info.st_mode):
...
def verify_evidence(repo: Path, files: list[EvidenceFile]) -> None:
    for record in files:
        try:
            ...
            raise SystemExit(
                "evidence changed or became unsafe; rerun autoreview"
            ) from exc
        if current != record:
            raise SystemExit(
                "evidence changed; rerun autoreview against the updated inputs"
            )
...
def compact_review_chunk_context(context: list[str]) -> list[str]:
    ...
    path_header = (
        new_header if new_header and new_header != "+++ /dev/null\n" else old_header
    )
...
def review_chunk_context(
    ...
    *,
    ...
    if context and context[0] == "# Untracked File\n" and next_new_line is not None:
        lines.append(
            f"[Continuation begins at untracked source line {next_new_line}.]\n"
        )
    elif (
...
def split_oversized_review_unit(
    ...
    for line in literal_lf_lines(unit):
        ...
        diff_line_marker = (
            line[0] if in_hunk and line.startswith(("+", "-", " ")) else None
        )
        ...
        if not current:
            current_context = review_chunk_context(
                context, next_new_line, next_old_line
            )
        current.append(line)
...
def mixed_capacity_error(
    record: MixedPath, required: int, limit: int
) -> MixedContextCapacityError:
    sizes = ", ".join(
        f"{name}={utf8_size(source.content or '')} bytes"
        for name, source in (
            ("index", record.index),
            ("working_tree", record.working_tree),
        )
...
def build_mixed_change_passes(
    ...
    prompt = render_review_prompt(
        branch, target, target_ref, full, extra_prompt, rendered
    )
    ...
    for span in captured.spans:
        ...
        ranges.append(
            (span.start, span.end, (records[span.path],), ((span.path, span.target),))
        )
        offset = span.end
    if offset < len(data):
    ...
    for start, end, sources, transitions in ranges:
        ...
        while limit >= 4:
            ...
            for piece in pieces:
                owned.append(
                    piece._replace(
                        byte_offset=position, sources=sources, transitions=transitions
                    )
                )
...
def build_mixed_review_passes(
    ...
    full = mixed_bundle_chunk(captured)
    prompt = render_review_prompt(
        branch, target, target_ref, full, extra_prompt, render_datasets(datasets)
    )
    if utf8_size(prompt) <= max_prompt_bytes:
        return [ReviewPass(prompt, full, tuple(datasets))]
    ...
    minimum_evidence = max(
        utf8_size(
            render_datasets([ReviewDataset(item.path, "", utf8_size(item.content))])
        )
        + 4
...
def build_change_review_prompts(
    ...
    content_limit = max_prompt_bytes - utf8_size(empty_chunk_prompt) - 4_096
    if content_limit < 4:
        return None
...
def build_review_prompts(
    ...
    if isinstance(bundle, CapturedBundle):
        if bundle.mixed:
            return build_mixed_review_passes(
                repo,
                target,
                target_ref,
                bundle,
    ...
    available = max_prompt_bytes - utf8_size(fixed_prompt) - 4_096
    batch_policy = (
    ...
    evidence_limit = (
        available
        - min(utf8_size(bundle), available // 2)
...
def toml_inline_string_table(values: dict[str, str]) -> str:
    entries = ", ".join(
        f"{toml_key(key)}={toml_string(value)}" for key, value in sorted(values.items())
    )
    return "{" + entries + "}"
...
def codex_config_isolation_flags(repo: Path, runtime_root: Path) -> list[str]:
    ...
    if sys.platform == "darwin":
        ...
        filesystem.update(
            {f"{root}{{,/**}}": "deny" for root in CODEX_MACOS_SCRATCH_ROOTS}
        )
...
def load_codex_auth_config(path: Path, *, strict: bool = False) -> dict[str, Any]:
    try:
        ...
        if strict and path.is_symlink():
            raise SystemExit(
                "Codex inference configuration refused: operator TOML is unavailable"
            )
        return {}
    except (OSError, UnicodeError):
        if strict:
            raise SystemExit(
                "Codex inference configuration refused: operator TOML is unreadable"
            )
        return {}
    try:
        try:
        ...
        if strict:
            raise SystemExit(
                "Codex inference configuration refused: reading an operator route requires Python 3.11 or tomli"
            )
    ...
    try:
        ...
        if strict:
            raise SystemExit(
                "Codex inference configuration refused: operator TOML is invalid"
            )
        return {}
...
def codex_auth_config_flags(
    ...
    *,
    ...
    flags: list[str] = ["-c", 'cli_auth_credentials_store="file"'] if force_file else []
    if not force_file:
    ...
    if isinstance(workspace_ids, str) and workspace_ids.strip():
        flags.extend(
            ["-c", f"forced_chatgpt_workspace_id={toml_string(workspace_ids.strip())}"]
        )
        ...
        if normalized_workspace_ids:
            flags.extend(
                [
                    "-c",
                    f"forced_chatgpt_workspace_id={toml_value(normalized_workspace_ids)}",
                ]
            )
    return flags
...
def codex_auth_helper_home(repo: Path) -> Path:
    try:
        ...
        while pending:
            ...
            seen.add(path)
            if any(
                is_within(path, root) or is_within(path.resolve(), root)
                for root in roots
            ):
...
def load_codex_inference_route(
    ...
    def external_path(
        ...
        *,
        ...
        if (
            ...
            or not value
            or (absolute and not Path(value).is_absolute())
        ):
            refuse(
                f"{label} must be {'an absolute ' if absolute else 'an '}external path"
            )
        try:
    ...
    if any(key in config for key in ("profile", "openai_base_url")):
        refuse(
            "profile or base URL overrides cannot be projected as one inference route"
        )
    if provider_id in {
        "openai",
        "ollama",
    ...
    command = external_path(
        auth.get("command"), "authentication executable", absolute=True
    )
    if not os.access(command, os.X_OK):
        refuse("authentication executable is not executable")
    if os.name != "nt":
        codex_auth_helper_home(repo)
    auth_cwd = external_path(
        auth.get("cwd", str(source_home)), "authentication cwd", directory=True
    )
    for key, minimum in (("timeout_ms", 1), ("refresh_interval_ms", 0)):
        if key in auth and (
            type(auth[key]) is not int or not minimum <= auth[key] <= (1 << 64) - 1
        ):
            refuse(f"invalid authentication {key}")
    if any(
        override.partition("=")[0].strip() in context_keys for override in overrides
    ):
    ...
    settings = {
        ...
        **{
            f"model_providers.{provider_id}.{key}": value
...
def prepare_codex_inference_config(
    route: tuple[list[str], bytes | None],
    runtime_root: Path,
) -> list[str]:
...
def stage_codex_auth_helper(
    repo: Path, runtime_root: Path, flags: list[str]
) -> list[str]:
    if os.name == "nt":
        return flags
    staged = list(flags)
    for index, flag in enumerate(flags):
        ...
        if not separator or not re.fullmatch(
            r"model_providers\.[A-Za-z0-9_-]+\.auth\.command", key
        ):
            continue
...
def ensure_claude_isolation_supported(args: argparse.Namespace, repo: Path) -> None:
    ...
    minimum_version = (
        CLAUDE_FABLE_MIN_VERSION if uses_fable else CLAUDE_SAFE_MODE_MIN_VERSION
    )
    version_reason = "for claude-fable-5" if uses_fable else "for --safe-mode"
    if result.returncode != 0:
        raise SystemExit(
            f"claude engine requires Claude Code >= {format_version(minimum_version)}; --version failed"
        )
    version = parse_cli_version(result.stdout or result.stderr)
    if version is None:
        raise SystemExit(
            f"claude engine requires Claude Code >= {format_version(minimum_version)} {version_reason}; could not parse --version output"
        )
    ...
    if help_result.returncode != 0 or missing:
        ...
        raise SystemExit(
            f"claude engine requires Claude Code isolation flags missing from --help: {detail}"
        )
...
def ensure_amp_isolation_supported(args: argparse.Namespace, repo: Path) -> str:
    ...
    if help_result.returncode != 0 or missing:
        detail = ", ".join(missing) if missing else "--help failed"
        raise SystemExit(
            "amp engine requires Amp CLI isolation flags missing from --help: " + detail
        )
    return amp_bin
...
def ensure_pi_isolation_supported(args: argparse.Namespace, repo: Path) -> str:
    ...
    if result.returncode != 0:
        raise SystemExit(
            f"pi engine requires Pi >= {format_version(PI_TRUST_ISOLATION_MIN_VERSION)}; --version failed"
        )
    version = parse_cli_version(f"{result.stdout}\n{result.stderr}")
    if version is None:
        raise SystemExit(
            f"pi engine requires Pi >= {format_version(PI_TRUST_ISOLATION_MIN_VERSION)} for --no-approve; could not parse --version output"
        )
    ...
    if help_result.returncode != 0 or missing:
        detail = ", ".join(missing) if missing else "--help failed"
        raise SystemExit(
            f"pi engine requires Pi isolation flags missing from --help: {detail}"
        )
    return pi_bin
...
def ensure_kimi_isolation_supported(args: argparse.Namespace, repo: Path) -> str:
    ...
    version = parse_cli_version(f"{version_result.stdout}\n{version_result.stderr}")
    if version is None:
        raise SystemExit(
            "kimi engine requires Kimi Code CLI >= "
...
def validate_kimi_runtime_auth_sources(
    ...
    try:
        ...
        device_id = ""
    if device_id:
        if is_within(resolved_device_id, repo.resolve()) or not re.fullmatch(
            r"[A-Za-z0-9-]{16,128}", device_id
        ):
    ...
    if not resolved_credentials.is_dir() or is_within(
        resolved_credentials, repo.resolve()
    ):
        raise SystemExit(
...
def prepare_kimi_runtime_auth(
    ...
    device_id, resolved_credentials = validate_kimi_runtime_auth_sources(
        repo, source_share
    )
    if source_share is None:
        return
...
def toml_value(value: Any) -> str:
    ...
    raise SystemExit(
        f"TOML config contains a value autoreview cannot serialize: {value!r}"
    )
...
def codex_config_overrides(args: argparse.Namespace) -> list[str]:
    ...
    for item in raw:
        ...
        if (
            ...
            raise SystemExit(
                f"invalid Codex config override (expected key=value): {item}"
            )
...
def codex_config_keys(args: argparse.Namespace) -> list[str]:
    return [
        override.partition("=")[0].strip() for override in codex_config_overrides(args)
    ]
...
def codex_model_access_failure(
...
def codex_failure_category(result: subprocess.CompletedProcess[str]) -> str:
    ...
    categories = (
        ...
        (
            "cli-arguments",
            ("unexpected argument", "unrecognized option", "unrecognized subcommand"),
        ),
        (
            "configuration",
            (
        ...
        (
            "rate-limit",
            ("429 too many requests", "http 429", "rate limit", "rate_limit"),
        ),
        (
            "transport",
            (
    ...
    *,
    ...
    if auth_config is None:
        auth_config = prepare_codex_inference_config(
            load_codex_inference_route(source_repo, overrides),
            runtime_root,
        )
...
def run_codex(args: argparse.Namespace, repo: Path, prompt: str) -> str:
    ...
    try:
        ...
        with (
            ...
            command_auth = any(
                flag.startswith("model_provider=") for flag in auth_config
            )
            ...
            for index, model in enumerate(models):
                ...
                result = run_with_heartbeat(
                    ...
                    stream_display=CodexStreamDisplay(suppress_diagnostics=command_auth)
                    if args.stream_engine_output
                    else None,
                ...
                try:
                    ...
                    raise ReviewerUnavailable(
                        "codex engine returned non-UTF-8 report",
                ...
                if result.returncode == 0:
                    if command_auth and not output.strip():
                        raise ReviewerUnavailable(
                            "codex engine failed: missing-report; provider diagnostics suppressed",
                            reason="invalid_report",
                            result=result,
                        )
...
def run_claude(args: argparse.Namespace, repo: Path, prompt: str) -> str:
    ...
    if args.tools:
        ...
        cmd.extend(
            ["--tools", claude_tool_inventory(args), "--allowedTools", allowed_tools]
        )
    else:
        cmd.extend(["--tools", ""])
    if args.stream_engine_output:
...
def attest_amp_stream(output: str, review_root: Path) -> bool:
    ...
    for line_number, raw_line in enumerate(output.splitlines(), 1):
        ...
        if not isinstance(event.get("type"), str) or event["type"] not in {
            "system",
            "user",
            "assistant",
            "result",
        ...
        if event.get("parent_tool_use_id") is not None:
            raise SystemExit(
                "amp isolation attestation failed: nested tool activity was observed"
            )
        events.append(event)
    ...
    if [event.get("type") for event in events] != [
        ...
        raise SystemExit(
            "amp isolation attestation failed: unexpected adapter event sequence"
        )
    init = init_events[0]
    ...
    if init.get("mcp_servers") != []:
        raise SystemExit(
            "amp isolation attestation failed: Amp exposed MCP servers to the outer agent"
        )
    raw_cwd = init.get("cwd")
    if (
        not isinstance(raw_cwd, str)
        or Path(raw_cwd).resolve(strict=False) != review_root.resolve()
    ):
        raise SystemExit(
            "amp isolation attestation failed: outer agent used an unexpected working directory"
        )

    user_events = [event for event in events if event.get("type") == "user"]
    if len(user_events) != 2:
        raise SystemExit(
            "amp isolation attestation failed: expected the trigger and one tool result"
        )
    ...
    if (
        ...
        raise SystemExit(
            "amp isolation attestation failed: outer trigger message was malformed"
        )
    content = first_message.get("content")
    if content != [{"type": "text", "text": AMP_OUTER_TRIGGER}]:
        raise SystemExit(
            "amp isolation attestation failed: outer user prompt was not the fixed trigger"
        )
    ...
    for event_index, event in enumerate(events):
        ...
        if message.get("role") != expected_role:
            raise SystemExit(
                "amp isolation attestation failed: message role was malformed"
            )
        blocks = message.get("content")
        if not isinstance(blocks, list):
            raise SystemExit(
                "amp isolation attestation failed: message content was malformed"
            )
        message_blocks[event_index] = []
        for block in blocks:
            if not isinstance(block, dict):
                raise SystemExit(
                    "amp isolation attestation failed: message block was malformed"
                )
    ...
    if len(tool_uses) != 1 or any(
        ...
        raise SystemExit(
            "amp isolation attestation failed: adapter tool call was misplaced"
        )
    if (
        len(tool_result_blocks) != 1
        or tool_result_blocks[0].get("type") != "tool_result"
    ):
        raise SystemExit(
            "amp isolation attestation failed: adapter tool result was misplaced"
        )
    if any(block.get("type") not in {"text", "thinking"} for block in final_blocks):
        raise SystemExit(
            "amp isolation attestation failed: final response contained tool activity"
        )
    ...
    if (
        ...
        raise SystemExit(
            "amp isolation attestation failed: adapter tool call was malformed"
        )
    if tool_result.get("tool_use_id") != tool_use_id:
        raise SystemExit(
            "amp isolation attestation failed: adapter tool result did not match its call"
        )
    tool_succeeded = tool_result.get("is_error") is False
    if tool_succeeded and tool_result.get("content") != "Adapter completed.":
        raise SystemExit(
            "amp isolation attestation failed: adapter success result was malformed"
        )
    if not tool_succeeded and (
        ...
        raise SystemExit(
            "amp isolation attestation failed: adapter failure result was not sanitized"
        )

    result_events = [event for event in events if event.get("type") == "result"]
    if len(result_events) != 1:
        raise SystemExit(
            "amp isolation attestation failed: expected exactly one terminal result event"
        )
    terminal = result_events[0]
    if terminal.get("subtype") != "success" or terminal.get("is_error") is not False:
        raise SystemExit(
            "amp isolation attestation failed: outer Amp turn did not succeed"
        )
...
def run_amp_mcp_denial_preflight(
    ...
    if probe_root.exists():
        raise SystemExit(
            "amp MCP isolation preflight failed: unable to remove the probe skill"
        )
...
def amp_review_result(
    ...
    try:
        ...
        if len(result.stdout) > AMP_MAX_OUTPUT_CHARS:
            raise ReviewerUnavailable(
                "amp engine stream exceeds the output limit",
                reason="invalid_report",
                result=result,
            )
        ...
        if not tool_succeeded:
            raise ReviewerUnavailable(
                "amp adapter tool failed without producing an error file", result=result
            )
        if result.returncode != 0:
            detail = result.stderr or result.stdout
            raise ReviewerUnavailable(
        ...
        raise ReviewerUnavailable(
            str(exc), reason="runtime_validation_failed", result=result
        ) from None
...
def run_pi(args: argparse.Namespace, repo: Path, prompt: str) -> str:
    ...
    if result.returncode != 0:
        raise ReviewerUnavailable(
            f"pi engine failed ({result.returncode})\n{result.stderr or result.stdout}",
            result=result,
        )
    return result.stdout
...
class CodexStreamDisplay:
    def __init__(
        self, *, activity_seconds: int = 20, suppress_diagnostics: bool = False
    ) -> None:
    ...
    def __call__(self, name: str, line: str) -> str | None:
        if name != "stdout":
            return (
                ...
                else stream_display_escape(line)
            )
        try:
            event = json.loads(line)
        except json.JSONDecodeError:
        ...
        if event_type == "thread.started":
            return self.visible(
                f"codex thread: {event.get('thread_id', '<unknown>')}\n"
            )
...
def claude_tool_rules(args: argparse.Namespace) -> list[str]:
    tools = [
        tool.strip() for tool in args.claude_allowed_tools.split(",") if tool.strip()
    ]
    if not args.web_search:
        tools = [
            tool
            for tool in tools
            if claude_tool_name(tool) not in {"WebSearch", "WebFetch"}
        ]
    return tools
...
def extract_json(text: str) -> dict[str, Any]:
    ...
    try:
        ...
        raise SystemExit(
            f"review engine returned non-JSON output: {exc}\n{stripped[:2000]}"
        )
    ...
    if isinstance(parsed, dict) and isinstance(parsed.get("result"), str):
        ...
        if isinstance(result_json, dict) and "findings" in result_json:
            return result_json
        raise SystemExit(
            f"review engine result was not structured JSON:\n{parsed['result'][:2000]}"
        )
...
def mixed_attribution_error(
    ...
    source = (
        (record.base if target == "index" else record.index)
        if removed
        else getattr(record, target)
    )
    ...
    if removed:
        lines = dict(
            record.index_removed if target == "index" else record.working_tree_removed
        )
    ...
    if (
        (not excerpt and (text != "" or column != 0))
        or "\n" in excerpt
        or text[column : column + len(excerpt)] != excerpt
    ):
...
def _validate_report(
    ...
    if extra_top:
        raise SystemExit(
            f"review JSON has unexpected top-level keys: {sorted(extra_top)}"
        )
    ...
    if not isinstance(report.get("overall_correctness"), str) or report[
        "overall_correctness"
    ] not in {"patch is correct", "patch is incorrect"}:
        raise SystemExit(
            f"review JSON has invalid overall_correctness: {report.get('overall_correctness')}"
        )
    if (
        not isinstance(report.get("overall_explanation"), str)
        or not report["overall_explanation"]
    ):
    ...
    for index, finding in enumerate(report["findings"]):
        ...
        if extra_finding:
            raise SystemExit(
                f"finding {index} has unexpected keys: {sorted(extra_finding)}"
            )
        ...
        if set(location) != allowed_location:
            raise SystemExit(
                f"finding {index} has invalid code_location keys: {sorted(location)}"
            )
        ...
        normalized_rel = (
            raw_rel if raw_rel in changed_paths else raw_rel.replace("\\", "/")
        )
    ...
    if ignored_findings:
        ...
        report["scope_rejected_findings"] = [
            finding for _, finding, _, _ in ignored_findings
        ]
...
def validate_report(
    ...
    try:
        ...
        if isinstance(exc.code, str):
            raise SystemExit(display_escape(exc.code, 4000, multiline=True)) from None
        raise
...
def review_status(report: dict[str, Any]) -> str:
    if (
        report.get("scope_rejected_findings")
        or report.get("missing_required_findings")
...
def print_findings(findings: list[dict[str, Any]]) -> None:
    for finding in findings:
        ...
        if attribution := finding.get("source_attribution"):
            target = (
                "INDEX-only" if attribution["target"] == "index" else "WORKING_TREE"
            )
            print(
                f"target: {target}; side: {attribution['side']}; "
...
def print_report(report: dict[str, Any], *, label: str = "autoreview") -> None:
    ...
    if status == "incomplete":
        ...
        print(
            f"{display_label} filtered: no findings at the requested priority; not a correctness certificate"
        )
    elif findings:
        print(f"{display_label} findings: {len(findings)}")
    elif report["overall_correctness"] == "patch is incorrect":
        print(f"{display_label} verdict: patch is incorrect without discrete findings")
    else:
        print(
    ...
    for key, description in (
        (
            "scope_rejected_findings",
            "Rejected out-of-scope findings (retained for audit)",
        ),
        (
            "attribution_rejected_findings",
            "Rejected source attributions (retained for audit)",
        ),
        (
            "priority_filtered_findings",
            "Findings below the requested priority (retained for audit)",
        ),
    ):
        if report.get(key):
    ...
    for entry in report.get("pass_reports", []):
        ...
        print(
            f"{display_escape(entry['label'], 200)} overall: "
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--base",
        help="Branch base, or explicit commit to compare with the index in local mode.",
    )
    parser.add_argument("--commit", default="HEAD")
    parser.add_argument(
        "--engine",
        choices=ENGINES,
        default=os.environ.get("AUTOREVIEW_ENGINE", "codex"),
    )
    parser.add_argument(
        "--model",
        action="append",
    ...
    parser.add_argument(
        "--thinking",
        action="append",
        help="Thinking/effort override or engine=level. Repeatable. Codex: none, minimal, low, medium, high, xhigh, max; gpt-6-astra requires low through max. Claude: low, medium, high, xhigh, max. Amp: none, low, medium, high, xhigh, max. Pi: off, minimal, low, medium, high, xhigh. Kimi: off, on.",
    )
    parser.add_argument(
        "--fallback-model",
        action="append",
    ...
    parser.add_argument(
        ...
        action="store_false",
        default=True,
        help="Disable tools for engines that support it. Amp, Pi, and Kimi always run without tools; Codex rejects no-tools review.",
    )
    parser.add_argument(
        "--no-web-search", dest="web_search", action="store_false", default=True
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--prompt", action="append", help="Additional review instruction text."
    )
    parser.add_argument(
        "--prompt-file",
        action="append",
        help="Additional review instruction file inside the repo (relative or absolute path).",
    )
    parser.add_argument(
        "--dataset",
        action="append",
        help="Extra evidence file to include in the review bundle.",
    )
    parser.add_argument(
        "--max-priority",
        choices=["P0", "P1", "P2", "P3"],
    ...
    parser.add_argument(
        "--output", help="Write human output to a file as well as stdout."
    )
    parser.add_argument("--json-output", help="Write validated structured review JSON.")
    parser.add_argument(
        "--status-output",
        help="Write a versioned status sidecar, including reviewer_unavailable; does not change report JSON or exit codes.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        action="append",
        default=[],
        help="Require finding text to contain this substring.",
    )
    parser.add_argument(
        "--expect-findings",
        action="store_true",
        help="Treat findings as success; for harness acceptance tests.",
    )
...
def parse_keyed_options(
    values: list[str] | None, option: str
...
def reviewer_args(args: argparse.Namespace) -> list[argparse.Namespace]:
    ...
    global_fallback, fallback_by_engine = parse_keyed_options(
        args.fallback_model, "fallback-model"
    )
    ...
    if unused_fallback_engines:
        ...
        raise SystemExit(
            f"--fallback-model specified for unselected reviewer: {engine_list}"
        )
    selected_non_claude_fallback = sorted(
        engine for engine in fallback_engines if engine != "claude"
    )
    if selected_non_claude_fallback:
        engine_list = ", ".join(selected_non_claude_fallback)
        raise SystemExit(
            f"--fallback-model is only supported for claude, not {engine_list}"
        )
    if (global_fallback or env_global_fallback) and engine != "claude":
        raise SystemExit(
            "--fallback-model is only supported for claude; no claude reviewer selected"
        )
    if getattr(args, "codex_config", None) and engine != "codex":
        raise SystemExit(
            "--codex-config is only supported for codex; no codex reviewer selected"
        )
    if getattr(args, "codex_speed", None) and engine != "codex":
        raise SystemExit(
            "--codex-speed is only supported for codex; no codex reviewer selected"
        )
    model = (
    ...
    if thinking and thinking not in thinking_levels:
        ...
        raise SystemExit(
            f"invalid thinking level for {thinking_target}: {thinking} (valid: {valid})"
        )
...
def resolve_engine_binary(
    reviewer: argparse.Namespace, repo: Path
) -> tuple[bool, str | None]:
    ...
    if bundle_ok and inputs_ok:
        try:
            with PreparationProgress("bundle/evidence preparation"):
                threshold_extra_prompt = apply_finding_threshold_prompt(
                    args, evidence.prompt
                )
...
def run_reviewer(
    ...
    paths = (
        changed_paths.paths
        if isinstance(changed_paths, CapturedBundle)
        else changed_paths
    )
    available = (
        {record.identity for record in prompt.chunk.sources}
        if isinstance(prompt, ReviewPass)
        else set()
    )
...
def merge_chunk_reports(reports: list[tuple[str, dict[str, Any]]]) -> dict[str, Any]:
        ...
        for finding in chunk_report["findings"]:
            ...
            if attribution:
                ...
                variant = next(
                    (
                        item
                        for item in merged["claim_variants"]
                        if item["body"] == finding["body"]
                    ),
                    None,
                )
                if variant is None:
    ...
    report = {
        ...
        "overall_explanation": bounded_field(
            f"Review passes returned. {summary}. See preserved pass reports for provider conclusions.",
            3000,
        ),
        ...
        "pass_reports": [
            {"label": label, "report": copy.deepcopy(chunk_report)}
            for label, chunk_report in reports
        ],
    }
    if len(reports) == 1:
    ...
    for field in (
        ...
        retained = [
            ...
            for _, chunk_report in reports
            for finding in chunk_report.get(field, [])
        ]
        if retained:
    ...
    for option, value in (
        ...
        if getattr(args, "status_output", None):
            for previous_option, previous in outputs:
                ...
                same = (
                    unicodedata.normalize("NFD", str(resolved)).casefold()
                    == unicodedata.normalize("NFD", str(previous)).casefold()
                )
                if (
                    not same
                    and resolved.parent.exists()
                    and previous.parent.exists()
                ...
                if same:
                    raise SystemExit(
                        f"{option} must use a different path from {previous_option}"
                    )
...
def write_review_status(
    ...
    status: str,
    exit_code: int,
    failure: ReviewerUnavailable | None = None,
) -> None:
    ...
    atomic_write_text(
        Path(args.status_output).expanduser(), json.dumps(envelope, indent=2) + "\n"
    )
...
def main_impl() -> int:
    ...
    print(
        f"bundle: {utf8_size(captured.text)} bytes; review passes: {len(prompts)}",
        flush=True,
    )
