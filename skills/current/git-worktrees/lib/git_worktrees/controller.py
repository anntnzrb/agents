...
if TYPE_CHECKING:
    ...
    from .models import LeaseState, Mode

from .errors import InputError, RefusalError
from .models import Handoff, Lease
...
class Controller:
    ...
    def __init__(self, root: Path | None = None) -> None:
        """Use the fixed root unless an explicit one is given."""
        self.root: Path = DEFAULT_ROOT if root is None else root.expanduser()

    @property
    def control_dir(self) -> Path:
        """Return the versioned control directory."""
        return self.root / ".control" / "v1"

    @property
    def database_path(self) -> Path:
        """Return the SQLite state path."""
    ...
    def state_exists(self) -> bool:
        """Check the state database leaf."""
        return self._regular_leaf_exists(
            self.database_path, label="Controller state database"
        )

    def initialize(self) -> None:
        """Create the control tree with private permissions."""
        ...
        for directory in (self.root / ".control", self.control_dir, self.lock_dir):
            try:
                ...
                raise RefusalError(
                    "controller_path_unsafe",
                    "Controller path cannot be created",
                    {"path": str(directory)},
                ) from error
        ...
        with self.connect(write=True) as connection:
            _ = connection.executescript(
                ...
                CREATE INDEX IF NOT EXISTS leases_common_git_dir_idx
                ON leases(common_git_dir);
                ...
                CREATE INDEX IF NOT EXISTS handoffs_lease_state_idx
    ...
    def _ensure_safe_root(self) -> None:
        ...
        try:
            ...
            raise RefusalError(
                ...
                "The fixed worktree root cannot be resolved",
                {"path": str(root)},
            ) from error
        if physical != root:
            raise RefusalError(
                "allocation_root_symlink",
                "The fixed worktree root traverses a symlink",
                {"path": str(root)},
            )
        if root.exists() and root.is_symlink():
            raise RefusalError(
                "allocation_root_symlink",
                "The fixed worktree root is a symlink",
                {"path": str(root)},
            )
        try:
            ...
            raise RefusalError(
                "allocation_root_unsafe",
                "The fixed worktree root cannot be created",
                {"path": str(root)},
            ) from error
        try:
            ...
            raise RefusalError(
                "allocation_root_unsafe",
                "The fixed worktree root cannot be inspected",
                {"path": str(root)},
            ) from error
        if (
            root.is_symlink()
            ...
            or (
                sys.platform != "win32"
                and (root_stat.st_mode & 0o022 or root_stat.st_uid != os.getuid())
            )
        ):
            raise RefusalError(
                "allocation_root_unsafe",
                "The fixed worktree root is not a private real directory",
                {"path": str(root)},
            )
    ...
    def _validate_private_directory(path: Path) -> None:
        try:
            ...
            raise RefusalError(
                "controller_path_unsafe",
                "Controller path cannot be inspected",
                {"path": str(path)},
            ) from error
        if stat.S_ISLNK(directory_stat.st_mode) or not stat.S_ISDIR(
            ...
            raise RefusalError(
                "controller_path_unsafe",
                "Controller path is not a real directory",
                {"path": str(path)},
            )
        if sys.platform != "win32" and (
            directory_stat.st_mode & 0o022 or directory_stat.st_uid != os.getuid()
        ):
            raise RefusalError(
                "controller_path_unsafe",
                "Controller path is writable by another user",
                {"path": str(path)},
            )

    def connect(self, *, write: bool) -> sqlite3.Connection:
        """Connect to the state database, optionally for writing."""
        database_exists = self._regular_leaf_exists(
            self.database_path, label="Controller state database"
        )
        if write or database_exists:
            # sqlite3 connects by pathname; the private chain guards this leaf.
            for directory in (self.root, self.root / ".control", self.control_dir):
                self._validate_private_directory(directory)
        if write:
            connection = sqlite3.connect(
                str(self.database_path), isolation_level=None, timeout=30.0
            )
            ...
            if not database_exists:
                raise InputError(
                    "lease_unknown", "No controller state exists for this lease", {}
                )
            connection = sqlite3.connect(
                f"file:{quote(str(self.database_path))}?mode=ro",
                uri=True,
    ...
    def repository_lock(self, common_git_dir: Path) -> Generator[None, None, None]:
        ...
        _ = self._regular_leaf_exists(path, label="Repository controller lock")
        ...
        try:
            ...
            if error.errno in {errno.ELOOP, errno.EMLINK}:
                raise RefusalError(
                    "controller_path_unsafe",
                    "Repository controller lock is a symlink",
                    {"path": str(path)},
                ) from error
            raise RefusalError(
                "controller_path_unsafe",
                "Repository controller lock cannot be opened",
                {"path": str(path)},
            ) from error
        acquired = False
        try:
            if not stat.S_ISREG(os.fstat(descriptor).st_mode):
                raise RefusalError(
                    "controller_path_unsafe",
                    "Repository controller lock is not a regular file",
                    {"path": str(path)},
                )
            ...
            try:
                ...
                if time.monotonic() >= deadline:
                    raise RefusalError(
                        ...
                        "Timed out waiting for the repository controller lock",
                        {"path": str(path)},
                    ) from error
                time.sleep(0.05)
            else:
...
def row_to_lease(row: sqlite3.Row | tuple[object, ...]) -> Lease:
    """Convert a lease row into a Lease value."""
    (
        lease_id,
        common_git_dir,
        primary_path,
...
def row_to_handoff(row: sqlite3.Row | tuple[object, ...]) -> Handoff:
    """Convert a handoff row into a Handoff value."""
