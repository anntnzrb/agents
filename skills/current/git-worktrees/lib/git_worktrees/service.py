...
_LEASE_COLUMNS = (
    "lease_id, common_git_dir, primary_path, path, mode, branch, base, owner, "
    + "session_actor, task, state, provenance, owner_token_hash, created_at, "
    + "updated_at, released_at, failure"
)
_SELECT_LEASE_BY_ID = (
    "SELECT " + _LEASE_COLUMNS + " FROM leases WHERE lease_id = ?"  # noqa: S608
)
_SELECT_LEASES_BY_COMMON_DIR = (
    "SELECT "  # noqa: S608 - static columns concatenated below
    + _LEASE_COLUMNS
    + " FROM leases WHERE common_git_dir = ? ORDER BY created_at"
)
_SELECT_ACTIVE_HANDOFFS_BY_LEASE = (
    "SELECT handoff_id, lease_id, actor, session_actor, state, created_at, "  # noqa: S608 - static query
    + "completed_at FROM handoffs WHERE lease_id = ? AND state = 'active'"
)
...
def _require_text(value: object, field: str) -> None:
    if not isinstance(value, str) or not value or "\x00" in value:
        raise InputError(
            "invalid_argument",
            f"{field} must be a nonempty string without NUL",
            {"field": field},
        )
...
def _validate_acquire(request: AcquireRequest) -> None:
    for field, value in (
        ("owner", request.owner),
        ("session_actor", request.session_actor),
        ("task", request.task),
    ):
    ...
    if request.mode == "new-branch":
        if not request.base or request.branch is not None:
            raise InputError(
                ...
                "new-branch requires base and forbids branch",
                {},
            )
        ...
        if not request.branch or request.base is not None:
            raise InputError(
                "invalid_mode_arguments",
                "existing-branch requires branch and forbids base",
                {},
            )
        ...
        if not request.base or request.branch is not None:
            raise InputError(
                "invalid_mode_arguments",
                "detached-ephemeral requires base and forbids branch",
                {},
            )
    else:
        raise InputError("invalid_mode", "mode is invalid", {"mode": request.mode})
    for setup in request.setup:
        if not setup.argv or any(
            not argument or "\x00" in argument for argument in setup.argv
        ):
            raise InputError(
                "invalid_setup_argv",
                "each setup argv must be a nonempty array of nonempty strings",
                {},
            )
...
def _safe_path_chain(root: Path, path: Path) -> None:
    if root.is_symlink() or not root.is_dir():
        raise RefusalError(
            "allocation_root_unsafe",
            "The fixed worktree root is not a real directory",
            {"path": str(root)},
        )
    try:
        ...
        raise RefusalError(
            "allocation_outside_root",
            "Allocation is outside the fixed worktree root",
            {"path": str(path)},
        ) from error
    current = root
    for part in relative.parts:
        ...
        if current.is_symlink():
            raise RefusalError(
                "allocation_symlink",
                "Allocation path contains a symlink",
                {"path": str(current)},
            )
...
def _visible_slug(
    ...
    if bound is not None and str(bound[0]) != common:
        ...
        if alternate is not None and str(alternate[0]) != common:
            raise RefusalError(
                "namespace_collision",
                "Visible repository namespace belongs to another repository",
                {"slug": slug},
            )
    ...
    if parent.exists() and mapping is None:
        if not parent.is_dir():
            raise RefusalError(
                "namespace_unbound",
                "Visible repository namespace is not a directory",
                {"path": str(parent)},
            )
        try:
            ...
            raise RefusalError(
                "namespace_unbound",
                "Visible repository namespace cannot be inspected",
                {"path": str(parent)},
            ) from error
        if has_contents:
            raise RefusalError(
                "namespace_unbound",
                "Visible repository namespace is not bound by this controller",
                {"path": str(parent)},
            )
    _ = connection.execute(
        "INSERT INTO repository_names(visible_slug, common_git_dir, created_at) "
        + "VALUES (?, ?, ?)",
        (slug, common, utc_now()),
    )
    return slug
...
def _visible_slug_for_inspection(
...
def _allocate_destination(
    ...
    if parent.is_symlink():
        raise RefusalError(
            "allocation_symlink",
            "Visible repository namespace is a symlink",
            {"path": str(parent)},
        )
    occupied_refs = {
        worktree.ref for worktree in repository.worktrees if worktree.ref is not None
    }
    candidate = name
    index = 2
    while True:
        ...
        lease_path_exists = (
            connection.execute(
                "SELECT 1 FROM leases WHERE path = ? LIMIT 1", (str(destination),)
            ).fetchone()
            is not None
        )
        branch_taken = mode == "new-branch" and (
            f"refs/heads/{branch}" in occupied_refs
            or local_branch_exists(repository.primary_path, branch)
        )
        if (
            not destination.exists()
            and not destination.is_symlink()
            and not lease_path_exists
...
def _lease(connection: sqlite3.Connection, lease_id: str) -> Lease:
    ...
    if row is None:
        raise InputError(
            "lease_unknown",
            "Lease is not managed by this controller",
            {"lease_id": lease_id},
        )
...
def _mark_failure(
    controller: Controller, lease_id: str, state: str, error: DomainError
) -> None:
    with _write_transaction(controller) as connection:
        _ = connection.execute(
            """UPDATE leases SET state = ?, updated_at = ?, failure = ?
            WHERE lease_id = ?""",
...
def _raise_acquire_failure(
    ...
    if error.exit_code == _EXIT_COMMAND_NOT_FOUND:
        raise error
...
def _assert_fresh_identity(initial: Repository, refreshed: Repository) -> None:
    if (
        initial.common_git_dir != refreshed.common_git_dir
        or initial.primary_path != refreshed.primary_path
    ):
        raise RefusalError(
...
def _locked_lease_repository(
    ...
    if (
        initial_repository.common_git_dir != initial_lease.common_git_dir
        or initial_repository.primary_path != initial_lease.primary_path
    ):
        raise RefusalError(
            "repository_identity_changed",
            "Repository identity no longer matches the lease",
            {},
        )
...
def _assert_managed_target(repository: Repository, lease: Lease) -> GitWorktree:
    matched = [
        worktree for worktree in repository.worktrees if worktree.path == lease.path
    ]
    if len(matched) != 1:
        raise RefusalError(
            "worktree_unregistered",
            "Managed worktree is not uniquely registered",
            {"path": str(lease.path)},
        )
    target = matched[0]
    if target.path == repository.primary_path:
        raise RefusalError(
            "worktree_primary",
            "The primary worktree can never be released",
            {"path": str(target.path)},
        )
    if target.locked is not None or target.prunable is not None:
        raise RefusalError(
            "worktree_uncertain",
            "Managed worktree has Git lock or prunable annotations",
            {"path": str(target.path)},
        )
    ...
    if expected is None:
        if not target.detached or target.ref is not None:
            raise RefusalError(
                "worktree_ref_mismatch",
                "Detached worktree no longer matches its lease",
                {"path": str(target.path)},
            )
    elif target.ref != expected or target.detached:
        raise RefusalError(
            "worktree_ref_mismatch",
            "Worktree ref no longer matches its lease",
            {
                "path": str(target.path),
                "expected_ref": expected,
                "actual_ref": target.ref,
            },
        )
    return target
...
def _setup_command(command: SetupCommand, cwd: Path, timeout: int) -> dict[str, object]:
    ...
    try:
        ...
        raise GitError(
            "setup_start_failed",
            "Could not start setup command",
            {"argv": list(command.argv), "error": str(error)},
        ) from error
    limit = 64 * 1024
    ...
    if process.stdout is None or process.stderr is None:
        ...
        raise GitError(
            "setup_start_failed",
            "Could not capture setup command output",
    ...
    threads = [
        Thread(target=collect, args=(0, process.stdout), daemon=True),
        Thread(target=collect, args=(1, process.stderr), daemon=True),
    ]
...
def inspect_repository(
    repo: Path, controller: Controller | None = None
) -> dict[str, object]:
    ...
    if active_controller.state_exists():
        try:
            ...
            findings.append(
                {
                    "code": "controller_state_unreadable",
                    "message": "Controller state could not be read",
                    "details": {"error": str(error)},
                }
            )
    parent = active_controller.root / (
        visible_slug if visible_slug is not None else _namespace_slug(repository)
    )
    try:
        if parent.is_symlink() or (parent.exists() and not parent.is_dir()):
            findings.append(
                {
                    "code": "allocation_parent_unsafe",
                    "message": "Visible allocation parent is unsafe",
        ...
        findings.append(
            {
                "code": "allocation_parent_unreadable",
                "message": "Visible allocation parent cannot be inspected",
                "details": {"path": str(parent), "error": str(error)},
            }
        )
    return {
...
def acquire(controller: Controller, request: AcquireRequest) -> dict[str, object]:
    """Acquire a managed worktree lease."""
    ...
    with controller.repository_lock(initial.common_git_dir):
        ...
        with _write_transaction(controller) as connection:
            destination, candidate = _allocate_destination(
                connection, controller, repository, request.name, request.mode
            )
            branch = (
                f"work/{candidate}" if request.mode == "new-branch" else request.branch
            )
            ...
            if request.mode == "existing-branch":
                if branch is None:
                    raise InputError(
                        "invalid_branch", "Existing branch is required", {}
                    )
                if not local_branch_exists(repository.primary_path, branch):
                    raise RefusalError(
                        "branch_missing",
                        "Existing branch does not exist",
                        {"branch": branch},
                    )
                if any(
                    worktree.ref == f"refs/heads/{branch}"
                    for worktree in repository.worktrees
                ):
                    raise RefusalError(
                        "branch_attached",
                        "Existing branch is already attached to a worktree",
                        {"branch": branch},
                    )
            lease_id = str(uuid4())
            created_at = utc_now()
            _ = connection.execute(
                ...
                'reserved', 'created-by-lease', NULL, ?, ?, NULL, NULL)""",
                (
                    lease_id,
                    str(repository.common_git_dir),
        ...
        try:
            ...
            _ = _assert_managed_target(after_add, lease)
        except DomainError as error:
            _raise_acquire_failure(controller, lease_id, "create_failed", error)
        try:
            for command in request.setup:
                _ = _setup_command(command, destination, request.setup_timeout_seconds)
        ...
        with _write_transaction(controller) as connection:
            _ = connection.execute(
                """UPDATE leases SET state = 'ready', owner_token_hash = ?,
                updated_at = ? WHERE lease_id = ? AND state = 'reserved'""",
...
def status(controller: Controller, lease_id: str) -> dict[str, object]:
    ...
    observation: dict[str, object] = {
        ...
        "provenance_matches": lease.provenance == "created-by-lease",
        "matches_lease": False,
    }
    try:
        repository = git_inspect_repository(lease.primary_path)
        observation["identity_matches"] = (
            repository.common_git_dir == lease.common_git_dir
            and repository.primary_path == lease.primary_path
        )
        matching = [
            worktree for worktree in repository.worktrees if worktree.path == lease.path
        ]
        ...
        if len(matching) == 1:
            ...
            observation["matches_lease"] = (
                ...
                and (
                    (expected_ref is None and target.detached and target.ref is None)
                    or (
                        expected_ref is not None
                        and not target.detached
                        and target.ref == expected_ref
                    )
                )
            )
        ...
        blockers.append(
            {
                ...
                "message": "Lease is not in ready state",
                "details": {"state": lease.state},
            }
        )
    if active:
        blockers.append(
            {
                "code": "active_handoffs",
                "message": "Lease has active handoffs",
                "details": {"handoff_ids": [handoff.handoff_id for handoff in active]},
            }
        )
        ...
        blockers.append(
            {
                ...
                "message": "Repository identity does not match the lease",
                "details": {},
            }
        )
    if not bool(observation["registered"]) or bool(observation["primary"]):
        blockers.append(
            {
                "code": "worktree_registration_unsafe",
                "message": "Worktree is not a registered linked worktree",
                "details": {},
            }
        )
        ...
        blockers.append(
            {
                ...
                "message": "Managed worktree path is missing",
                "details": {"path": str(lease.path)},
            }
        )
    if observation["dirty"] is None:
        blockers.append(
            {
                "code": "worktree_state_uncertain",
                "message": "Worktree dirtiness could not be observed",
                "details": {},
            }
        )
    if not bool(observation["matches_lease"]):
        blockers.append(
            {
                "code": "worktree_ref_mismatch",
                "message": "Worktree does not match lease mode or ref",
                "details": {},
            }
        )
        ...
        blockers.append(
            {
                ...
                "message": "Lease provenance is not controller managed",
                "details": {},
            }
        )
    return {
        ...
        "active_handoffs": [handoff.public() for handoff in active],
        "blockers": blockers,
        "safe_to_release": not blockers,
    }


def _validate_owner(lease: Lease, token: str) -> None:
    _require_text(token, "owner_token")
    if lease.owner_token_hash is None or not hmac.compare_digest(
        lease.owner_token_hash, sha256(token.encode("utf-8")).hexdigest()
    ):
        raise RefusalError(
            "owner_capability_mismatch",
            "Owner capability does not match this lease",
...
def handoff(
    ...
    """Hand off a lease to another actor."""
    _require_text(lease_id, "lease_id")
    _require_text(actor, "actor")
    _require_text(session_actor, "session_actor")
    with (
        _locked_lease_repository(controller, lease_id) as repository,
        _write_transaction(controller) as connection,
    ):
        lease = _lease(connection, lease_id)
        ...
        if lease.state != "ready":
            raise RefusalError(
                "lease_not_ready",
                "Only a ready lease can be handed off",
                {"state": lease.state},
            )
        if (
            lease.common_git_dir != repository.common_git_dir
            or lease.primary_path != repository.primary_path
        ):
            raise RefusalError(
                "repository_identity_changed",
                "Repository identity no longer matches the lease",
        ...
        _ = _assert_managed_target(repository, lease)
        active = _active_handoffs(connection, lease_id)
        if active:
            raise RefusalError(
                "handoff_active",
                "A handoff is already active for this lease",
        ...
        _ = connection.execute(
            ...
            token_hash, state, created_at, completed_at)
            VALUES (?, ?, ?, ?, ?, 'active', ?, NULL)""",
            (
                handoff_id,
                lease_id,
        ...
        created = Handoff(
            handoff_id, lease_id, actor, session_actor, "active", now, None
        )
    return {
        "lease": lease.public(),
        "handoff": created.public(),
        "capabilities": {"handoff_token": token},
    }


def complete_handoff(
    controller: Controller, lease_id: str, handoff_token: str, quiescent: bool
) -> dict[str, object]:
    """Complete a handoff after quiescence attestation."""
    _require_text(lease_id, "lease_id")
    _require_text(handoff_token, "handoff_token")
    if quiescent is not True:
        raise RefusalError(
            "quiescence_required", "Completion requires a quiescence attestation", {}
        )
    ...
    with _write_transaction(controller) as connection:
        ...
        if row is None:
            raise RefusalError(
                "handoff_capability_mismatch",
                "No active handoff matches this capability",
                {"lease_id": lease_id},
            )
        now = utc_now()
        _ = connection.execute(
            """UPDATE handoffs SET state = 'completed', completed_at = ?
                WHERE handoff_id = ?""",
            (now, str(row[0])),
        )
        completed = row_to_handoff(
            (row[0], row[1], row[2], row[3], "completed", row[5], now)
        )
        lease = _lease(connection, lease_id)
...
def release(
    controller: Controller, lease_id: str, owner_token: str, quiescent: bool
) -> dict[str, object]:
    """Release a lease after quiescence attestation."""
    _require_text(lease_id, "lease_id")
    if quiescent is not True:
        raise RefusalError(
            "quiescence_required", "Release requires a quiescence attestation", {}
        )
    with (
        _locked_lease_repository(controller, lease_id) as repository,
        _write_transaction(controller) as connection,
        ...
        if lease.state != "ready":
            raise RefusalError(
                "lease_not_ready",
                "Only a ready lease can be released",
                {"state": lease.state},
            )
        if (
            lease.common_git_dir != repository.common_git_dir
        ...
        if lease.provenance != "created-by-lease":
            raise RefusalError(
                "provenance_mismatch",
                "Lease provenance is not controller managed",
                {},
            )
        active = _active_handoffs(connection, lease_id)
        if active:
            raise RefusalError(
                "handoff_active",
                "Active handoffs must be completed before release",
                {"handoff_ids": [handoff.handoff_id for handoff in active]},
            )
        _ = _assert_managed_target(repository, lease)
        ...
        if (
            any(worktree.path == lease.path for worktree in after.worktrees)
            or lease.path.exists()
        ):
            raise RefusalError(
                "worktree_remove_unconfirmed",
                "Git removal did not remove the managed worktree",
                {"path": str(lease.path)},
            )
        now = utc_now()
        _ = connection.execute(
            """UPDATE leases SET state = 'released', updated_at = ?,
            released_at = ? WHERE lease_id = ? AND state = 'ready'""",
            (now, now, lease_id),
        )
