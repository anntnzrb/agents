...
- `./pyproject.toml`: sync app metadata, dependencies, and tool config (ruff, basedpyright, pytest)
...
- Sync installs and controls only the per-user services it declares (systemd user units, launchd user agents), records them as owned, and prunes owned ones it no longer declares. It never touches system-level services or units it does not own.
