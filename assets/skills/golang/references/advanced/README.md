...
> **Precedence:** Treat this directory as an opinionated recipe library, not universal policy. Repository instructions, the `go`/`toolchain` directives, configured gates, existing dependencies, and nearby working code override its stack and style defaults.
...
When editing an existing `.go` file that does not follow these rules, write new code in the repository's established style unless the task explicitly includes a focused refactor. Keep branch-scope cleanup separate from feature work.
