...
Use the public `uv run --script <skill-dir>/scripts/cli.py ...` entrypoint. `uv`
selects the declared Python runtime on macOS, Linux, and Windows.
...
```bash
uv run --script <skill-dir>/scripts/cli.py "<product_type> <industry> <keywords>" --design-system [-p "Project Name"]
```
...
**Example:**
```bash
uv run --script <skill-dir>/scripts/cli.py "beauty spa wellness service" --design-system -p "Serenity Spa"
```
...
```bash
uv run --script <skill-dir>/scripts/cli.py "<query>" --design-system --persist -p "Project Name"
```
...
```bash
uv run --script <skill-dir>/scripts/cli.py "<query>" --design-system --persist -p "Project Name" --page "dashboard"
```
...
```bash
uv run --script <skill-dir>/scripts/cli.py "<query>" --design-system --variance <1-10> --motion <1-10> --density <1-10>
```
...
**Example:**
```bash
uv run --script <skill-dir>/scripts/cli.py "internal analytics dashboard" --design-system --variance 8 --motion 7 --density 8 -p "Ops Console"
```
...
```bash
uv run --script <skill-dir>/scripts/cli.py "<keyword>" --domain <domain> [-n <max_results>]
```
...
```bash
uv run --script <skill-dir>/scripts/cli.py "<keyword>" --stack <your-stack>
...
```bash
uv run --script <skill-dir>/scripts/cli.py "AI search tool modern minimal" --design-system -p "AI Search"
```
...
uv run --script <skill-dir>/scripts/cli.py "minimalism dark mode" --domain style

# Get UX best practices for search interaction and loading
uv run --script <skill-dir>/scripts/cli.py "search loading animation" --domain ux
```

### Step 4: Stack Guidelines

```bash
uv run --script <skill-dir>/scripts/cli.py "list performance navigation" --stack nextjs
```
...
uv run --script <skill-dir>/scripts/cli.py "fintech crypto" --design-system

# Markdown - best for documentation
uv run --script <skill-dir>/scripts/cli.py "fintech crypto" --design-system -f markdown
```
