...
If you are about to write prose, read the [style guide](styleguide.md) first. It is the source of truth for headings, admonitions, code blocks, capitalisation, and how Clan's docs handle tone.

:::admonition[Prerequisites]{type=note}
A working Clan checkout with Nix and direnv installed. See [Contributing](CONTRIBUTING.md) if you have not set those up yet.
:::
...
Clan's docs support a few markdown extensions beyond plain CommonMark. The full catalogue is in the [style guide](styleguide.md). The ones you will reach for most often:
...
**`direnv` does not activate when I `cd` into `docs/`.** Run `direnv allow` once. If the hook was never installed, follow step 2 of [Contributing](CONTRIBUTING.md) and restart your shell.
