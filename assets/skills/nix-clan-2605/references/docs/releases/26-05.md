...
## Table of Contents
- [Release 26.05](#release-2605)
- [Highlights](#highlights)
  - [Safer machine updates](#safer-machine-updates)
  - [Activate a specialisation on update](#activate-a-specialisation-on-update)
  - [Vars prompts reuse previous values](#vars-prompts-reuse-previous-values)
  - [ZeroTier IP detection](#zerotier-ip-detection)
  - [p2p-ssh-iroh management transport *(experimental)*](#p2p-ssh-iroh-management-transport-experimental)
  - [SSH agent forwarding](#ssh-agent-forwarding)
- [Upgrade Steps](#upgrade-steps)
  - [Bump your flake input](#bump-your-flake-input)
  - [Upgrade Zerotier](#upgrade-zerotier)
  - [After bumping the `clan-core` version](#after-bumping-the-clan-core-version)
- [Breaking Changes](#breaking-changes)
  - [ZeroTier now supports multiple instances](#zerotier-now-supports-multiple-instances)
  - [sshd now issues host certificates by default](#sshd-now-issues-host-certificates-by-default)
  - [`clan flakes create` got replaced by `clan init`](#clan-flakes-create-got-replaced-by-clan-init)
  - [`clan vms` removed](#clan-vms-removed)
  - [Important upstream breakages to know about](#important-upstream-breakages-to-know-about)
    - [systemd stage 1 (initrd) is now the default](#systemd-stage-1-initrd-is-now-the-default)
    - [Default D-Bus implementation switched to `dbus-broker`](#default-d-bus-implementation-switched-to-dbus-broker)
    - [Default kernel updated from 6.12 to 6.18](#default-kernel-updated-from-612-to-618)
    - [`fileSystems.<name>.fsType` is now mandatory](#filesystemsfstype-is-now-mandatory)
    - [`networking.resolvconf.enable` now defaults to `true`](#networkingresolvconfenable-now-defaults-to-true)
  - [`x86_64-darwin` support dropped](#x86_64-darwin-support-dropped)
- [CLI Changes](#cli-changes)
- [Clan Community](#clan-community)
- [Vars Backends](#vars-backends)
  - [Configurable secret recipients *(experimental)*](#configurable-secret-recipients-experimental)
- [Exports](#exports)
- [Services](#services)
- [Option Changes](#option-changes)
  - [`clan.core` options](#clancore-options)
  - [`clan` (flake) options](#clan-flake-options)
- [Templates](#templates)
  - [Disk templates](#disk-templates)
  - [Machine templates](#machine-templates)
  - [Clan templates](#clan-templates)
- [Chores](#chores)
  - [`clan.core.clanPkgs`](#clancoreclanpkgs)
- [Thanks](#thanks)
...
Pass `--no-check` to force past inhibitors when you know it is safe. See the [nixos-rebuild guide](../guides/nixos-rebuild.md).

### Activate a specialisation on update

`clan machines update --specialisation <name>` activates a specific NixOS specialisation instead of the default system. Use it to switch a machine between hardware or service variants. See the [specialisations guide](../guides/specialisations.md).
...
This is the infra-management layer, separate from your service mesh: iroh carries operator traffic (`clan machines update`), while a [mesh VPN](../guides/networking/mesh-vpn.md) connects machines so services can talk to each other. Our goal is to try iroh as the default management transport.
...
If you use ZeroTier, follow the [ZeroTier migration guide](../guides/migrations/zerotier.md)
...
The full step-by-step procedure is documented in the [ZeroTier migration guide](../guides/migrations/zerotier.md). Follow it before upgrading.
...
`clan vms run` and `clan vms inspect` have been removed, and `vm` is no longer a valid value for `clan machines build --format`. The `vm` vars backend was removed as well (see [Vars Backends](#vars-backends)).
...
Beyond the command removals in [Breaking Changes](#breaking-changes), several commands gained new flags and behavior
...
Clan's architecture lets you write your [own services](../guides/services/community.md), and the [community services](../services/community.md) collection continues to grow.
...
The set of [vars](https://clan.lol/docs/26.05/reference/cli/vars) backends has changed:
...
[Exports](../guides/services/exports.md) have been refactored. Services now declare what they export under `manifest.out`. Exports remain **experimental** — we're still collecting real-world feedback before stabilizing them. The available modules are:
...
All clan templates were modernized and better integrated into the [quick-start](../getting-started/quick-start.md) and `clan-cli`:
