...
def cmd_weapons(args) -> None:
    if args.all or not args.name:
        ...
        for name, w in STARTER_WEAPONS.items():
            print(f"  {name.title()}: {w['base_damage']} DMG, {w['str_scale']}/{w['dex_scale']} scaling, req {w['str_req']} STR/{w['dex_req']} DEX, {w['weight']} wt ({w['category']})")
        return
    ...
    if name in STARTER_WEAPONS:
        ...
        for stats_label, stats in [("base (10/10)", {"str": 10, "dex": 10}), ("20/20", {"str": 20, "dex": 20}), ("40/40", {"str": 40, "dex": 40})]:
            ar = weapon_ar(w, stats)
            print(f"  AR at {stats_label}: ~{ar}")
    else:
        print(f"  Weapon '{args.name}' not in starter dataset. Use live research for full weapon list.")
...
def cmd_calc(args) -> None:
    ...
    if name not in STARTER_WEAPONS:
        ...
        return
    w = STARTER_WEAPONS[name]
    stats = {"str": min(args.str, 99), "dex": min(args.dex, 99), "int": min(args.int, 99), "fth": min(args.fth, 99)}
    ar = weapon_ar(w, stats)
...
def cmd_areas(args) -> None:
    ...
    areas = [
        ...
        ("The Ringed City", 4, ["Halflight, Spear of the Church", "Slave Knight Gael", "Darkeater Midir"], True, 3),
    ]
    for name, phase, bosses, required, bonfires in areas:
        phase_label = ["Starter", "Early", "Mid", "Late", "DLC"][phase]
        req = "REQUIRED" if required else "optional"
        print(f"  [{phase_label}] {name} ({req}, {bonfires} bonfire{'s' if bonfires > 1 else ''})")
        if bosses:
            for b in bosses:
                print(f"    Boss: {b}")
...
def cmd_bosses(args) -> None:
    ...
    if args.required:
        all_bosses = [(bid, phase, req, sl, area) for bid, phase, req, sl, area in all_bosses if req]
    if args.area:
        q = args.area.lower()
        all_bosses = [(bid, phase, req, sl, area) for bid, phase, req, sl, area in all_bosses if q in area.lower()]
    if args.spoilers:
        for bid, phase, req, _, _ in all_bosses:
            print(f"  [{phase}] {bid.replace('_', ' ').title()} {'(required)' if req else '(optional)'}")
    else:
        for bid, phase, req, spoiler_level, _ in all_bosses:
            if spoiler_level <= 0 or is_known(bid):
                print(f"  [{phase}] {bid.replace('_', ' ').title()} {'(required)' if req else '(optional)'}")
            else:
                print(f"  [{phase}] A boss you haven't encountered yet {'(required)' if req else '(optional)'}")

def cmd_route(args) -> None:
    print("=== Route Planning ===\n")
    # Boss data: (id, phase, required, spoiler_level)
    all_bosses = [
        ("iudex_gundyr", "Starter", True, 0), ("vordt", "Starter", True, 0),
        ("greatwood", "Early", False, 1), ("sage", "Early", True, 1),
        ("deacons", "Early", True, 1), ("abyss_watchers", "Early", True, 1),
        ("wolnir", "Mid", True, 2), ("old_demon_king", "Mid", False, 2),
        ("pontiff", "Mid", True, 2), ("aldrich", "Mid", True, 2),
        ("yhorm", "Mid", True, 2), ("dancer", "Late", True, 2),
        ("oceiros", "Late", False, 3), ("champion_gundyr", "Late", False, 3),
        ("dragonslayer_armour", "Late", True, 3), ("twin_princes", "Late", True, 3),
        ("ancient_wyvern", "Late", False, 3), ("nameless_king", "Late", False, 3),
        ("soul_of_cinder", "Late", True, 3), ("sister_friede", "DLC", True, 3),
        ("demon_prince", "DLC", True, 3), ("halflight", "DLC", True, 3),
        ("gael", "DLC", True, 3), ("midir", "DLC", False, 3),
    ...
    if args.defeated:
        ...
        required_bosses = [(bid, phase, sl) for bid, phase, req, sl in all_bosses if req]
        next_bosses = [(bid, phase, sl) for bid, phase, sl in required_bosses if bid not in defeated_ids]
        if next_bosses:
            ...
            if args.spoilers:
                ...
                for bid, phase, sl in next_bosses[:3]:
                    if phase not in hinted:
                        hinted.add(phase)
                        print(f"    → The next required boss awaits in {phase_hints.get(phase, 'an upcoming area')}")
        else:
    ...
    if not args.spoilers:
        ...
        return
    print("  [Spoiler route omitted — too long for CLI. Use Fextralife Game Progress Route.]")
...
def cmd_achievements(args) -> None:
    if args.missable:
        ...
        print(f"    1. To Link the First Flame — walk to bonfire after final boss (default)")
        print(f"    2. The End of Fire — give Fire Keeper Eyes to Fire Keeper, summon her at end")
        print(f"    3. The Usurpation of Fire — Yuria's questline, need 8 Dark Sigils")
        print(f"    4. Unkindled Ending — attack Fire Keeper during End of Fire cutscene")
        print(f"  Tip: Save-scum at the final boss for all 3 achievement endings from one kill.")
...
def cmd_spells(args) -> None:
    ...
    type_key = {"sorcery": "sorceries", "miracle": "miracles", "pyromancy": "pyromancies"}
    ...
    if args.achievement:
        ...
        for cat_key, cat_label in [("sorceries", "Sorceries"), ("miracles", "Miracles"), ("pyromancies", "Pyromancies")]:
            ...
            for s in spells:
                ...
                loc = spoiler_safe(s["location"]) if not is_known(s["name"]) else s["location"]
    ...
    if args.type:
        ...
        for s in filtered:
            ...
            if not reqs:
                reqs = "—"
            print(f"  {s['name']}{cov_mark}  [{reqs}]  {s['slots']} slot(s) — {s['fp_cost']} FP")
            print(f"    {s['effect']}")
    ...
    if args.name:
        ...
        for cat_key, cat_label in [("sorceries", "Sorcery"), ("miracles", "Miracle"), ("pyromancies", "Pyromancy")]:
            for s in SPELLS[cat_key]:
                if s["name"].lower() == target or target in s["name"].lower():
                    ...
                    if s["covenant_locked"]:
                        print(f"  ⚠ Covenant-locked — requires online or offline farming")
                    return
        print(f"Spell '{args.name}' not found. Try: spells (to list all) or spells <partial-name>")
        return
    ...
    print(f"  Use `spells <name>` for details, `spells --type` to filter, `spells --achievement` for plat list")
...
def cmd_rings(args) -> None:
    ...
    print("=== Rings Catalog ===\n")
    category_order = ["hp", "stamina", "equip_load", "defense", "elemental", "damage",
    ...
    for cat in category_order:
        ...
        print()
    print(f"Total: {len(RINGS)} rings. Use 'rings <name>' for detail, 'rings --build <type>' to filter.")
...
def cmd_npcs(args) -> None:
    if args.name:
        ...
        if matches:
            ...
            return
        print(f"NPC '{args.name}' not found. Known NPCs: {', '.join(sorted(NPC_QUESTS.keys()))}")
        return
    if args.missable:
