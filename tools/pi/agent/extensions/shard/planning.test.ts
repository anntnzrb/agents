...
describe("shard task planning", () => {
	...
	test("multiple tasks + omitted mode derives explorer", () => {
		...
		expect(plan.tasks.map((task) => task.childMode)).toEqual(["explorer", "explorer"]);
	});

	test("explicit explorer with one task is valid", () => {
		const plan = unwrap(buildTaskPlan({ mode: "explorer", tasks: ["inspect"] }, "/repo"));
		expect(plan.childMode).toBe("explorer");
		expect(plan.mode).toBe("single");
	});

	test("explicit explorer with multiple tasks is valid", () => {
		const plan = unwrap(buildTaskPlan({ mode: "explorer", tasks: ["a", "b"] }, "/repo"));
		expect(plan.childMode).toBe("explorer");
		expect(plan.tasks).toHaveLength(2);
	});

	test("explicit worker with one task is valid", () => {
		const plan = unwrap(buildTaskPlan({ mode: "worker", tasks: ["fix"] }, "/repo"));
		expect(plan.childMode).toBe("worker");
		expect(plan.tasks).toHaveLength(1);
	});

	test("explicit worker with multiple tasks is rejected", () => {
		const plan = buildTaskPlan({ mode: "worker", tasks: ["fix a", "fix b"] }, "/repo");
		expect(plan.ok).toBe(false);
		if (!plan.ok) expect(plan.error).toContain("worker mode accepts exactly one task");
	});
	...
	test("empty task is rejected", () => {
		...
		if (!result.ok) expect(result.error).toBe("tasks[1] must be a non-empty string.");
	});

	test("too many tasks rejected", () => {
		const result = normalizeTasks(Array.from({ length: 9 }, (_, index) => `task ${index}`));
		expect(result.ok).toBe(false);
	...
	test("invalid mode rejected", () => {
		...
		expect(result.ok).toBe(false);
		if (!result.ok) expect(result.error).toBe('Invalid mode "reviewer". Expected "worker" or "explorer".');
	});
});

describe("shard runtime tool selection", () => {
	test("explorer tool filter returns only active read grep find", () => {
		expect(selectRuntimeTools("explorer", ["bash", "read", "edit", "grep", "find", "write"])).toEqual([
			"read",
			"grep",
			"find",
		]);
	...
	test("explorer tool filter preserves active-tool order", () => {
		expect(selectRuntimeTools("explorer", ["find", "read", "grep"])).toEqual(["find", "read", "grep"]);
	});
...
describe("shard timeout validation", () => {
	...
	test("invalid timeout rejected", () => {
		...
		expect(result.ok).toBe(false);
		if (!result.ok) expect(result.error).toBe("timeoutSec must be a positive finite number of seconds.");
	});
