...
const PROMPT_SNIPPET =
  "Delegate bounded independent subtasks to child Pi workers";
...
const ToolParams = Type.Object({
  ...
  maxTurns: Type.Optional(
    Type.Number({
      description:
        "Optional per-child maximum assistant turns; no default. Use only for explicit budget limits.",
    }),
  ),
  maxToolCalls: Type.Optional(
    Type.Number({
      description:
        "Optional per-child maximum tool calls; no default. Use only for explicit budget limits.",
    }),
  ),
});
...
type TaskPlan = {
  ...
  childMode: "worker" | "explorer";
  tasks: TaskSpec[];
};

const createEmptyDetails = (
  childMode: "worker" | "explorer" = "worker",
): ToolDetails => ({
  mode: "single",
  childMode,
  results: [],
  ...
  pi.registerTool({
    ...
    async execute(_toolCallId, params: ToolParamsInput, signal, onUpdate, ctx) {
      ...
      const cwdError = await validateCwd(
        taskPlan.value.tasks[0]?.cwd ?? ctx.cwd,
      );
      if (cwdError) {
        return {
      ...
      const runtimeTools = selectRuntimeTools(
        taskPlan.value.childMode,
        pi.getActiveTools(),
      );
