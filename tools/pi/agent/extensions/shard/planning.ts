...
export const normalizeTasks = (rawTasks: unknown): PlanResult<string[]> => {
  if (!Array.isArray(rawTasks) || rawTasks.length === 0) {
    return {
      ok: false,
      error: "Provide tasks with at least one non-empty task string.",
    };
  }
  ...
  for (let index = 0; index < rawTasks.length; index += 1) {
    ...
    if (typeof task !== "string") {
      return {
        ok: false,
        error: `tasks[${index}] must be a non-empty string.`,
      };
    }
...
export const normalizeChildMode = (
  ...
  return {
    ok: false,
    error: `Invalid mode "${String(rawMode)}". Expected "worker" or "explorer".`,
  };
};

export const validateTimeoutSec = (
  value: unknown,
  ...
  if (typeof value !== "number" || !Number.isFinite(value) || value <= 0) {
    return {
      ok: false,
      error: "timeoutSec must be a positive finite number of seconds.",
    };
  }
  if (value > MAX_TIMEOUT_SEC) {
    return {
      ok: false,
      error: `timeoutSec must be <= ${MAX_TIMEOUT_SEC} seconds.`,
    };
  }
...
export const validateMaxTurns = (
  value: unknown,
): PlanResult<number | undefined> =>
  validatePositiveIntegerBudget(value, "maxTurns", MAX_TURNS);

export const validateMaxToolCalls = (
  value: unknown,
): PlanResult<number | undefined> =>
