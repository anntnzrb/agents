...
const getInputTokens = (usage?: AssistantUsage): number => getNumber(usage?.input) || getNumber(usage?.inputTokens);

const getOutputTokens = (usage?: AssistantUsage): number => getNumber(usage?.output) || getNumber(usage?.outputTokens);
...
const summarizeGcStats = (entries: readonly unknown[]): GcStats => {
	...
	for (const entry of entries as ContextGcEntry[]) {
		if (entry.type !== "custom" || entry.customType !== CONTEXT_GC_INDEX_TYPE) continue;
		const records = Array.isArray(entry.data?.records) ? entry.data.records : [];
		if (records.length === 0) continue;
		events += 1;
		for (const record of records) {
			if (typeof record.resultText === "string") rawChars += record.resultText.length;
		}
	}
	return { events, estimatedTokens: Math.ceil(rawChars / APPROX_CHARS_PER_TOKEN) };
};

const formatGcStats = (stats: GcStats): string => (stats.events > 0 ? ` · 🧹 ${stats.events} ~${formatCompactNumber(stats.estimatedTokens)}t` : "");
