...
describe("turn-stats helpers", () => {
	...
	test("summarizes context-gc index entries as gc events and estimated tokens", () => {
		...
		expect(__test.summarizeGcStats(entries)).toEqual({ events: 2, estimatedTokens: 3 });
	});
	...
	test("appends broom stats as gc-event count plus estimated tokens", () => {
		...
		expect(__test.formatGcStats({ events: 12, estimatedTokens: 53_000 })).toBe(" · 🧹 12 ~53kt");
		expect(__test.formatTurnStats({ input: 1000, output: 50 }, 1000, { events: 12, estimatedTokens: 53_000 })).toContain("🧹 12 ~53kt");
	});
});
