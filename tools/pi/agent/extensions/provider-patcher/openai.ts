...
const OPENAI_RESPONSES_APIS = new Set<string>(["openai-responses", "openai-codex-responses"]);

const isPayload = (value: unknown): value is Payload => typeof value === "object" && value !== null && !Array.isArray(value);

export const supportsOpenAIServiceTier = (model: ModelLike | undefined): boolean => !!model && OPENAI_RESPONSES_APIS.has(model.api);

export const effectiveOpenAIServiceTier = (model: ModelLike | undefined, tier: ServiceTier): ServiceTier => {
	...
	if (model?.api === "openai-codex-responses" && tier !== "priority") return "off";
	return tier;
};

export const supportsOpenAIServiceTierValue = (model: ModelLike | undefined, tier: ServiceTier): boolean =>
	effectiveOpenAIServiceTier(model, tier) === tier;

export const isOpenAIResponsesRequest = (model: ModelLike | undefined, payload: unknown): boolean => {
	...
	return typeof payload["model"] === "string" && ("input" in payload || "messages" in payload);
};
