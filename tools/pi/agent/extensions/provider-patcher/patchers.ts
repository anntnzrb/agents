...
import {
  ...
  supportsOpenAIServiceTierValue,
} from "./openai.js";
import type {
...
const OPENAI_TIER_CHOICES: readonly TierChoice[] = [
  { provider: "openai", tier: "off", label: "off — do not send service_tier" },
  {
    provider: "openai",
    tier: "default",
    label: "default — normal OpenAI routing",
  },
  {
    provider: "openai",
    tier: "flex",
    label: "flex — cheaper/slower when supported",
  },
  {
    provider: "openai",
    tier: "priority",
    label: "priority — faster OpenAI routing",
  },
];
...
const PATCHERS: readonly ProviderOptionsPatcher[] = [
  {
    ...
    getTierChoices: (model) =>
      supportsOpenAIServiceTier(model)
        ? OPENAI_TIER_CHOICES.filter((choice) =>
            supportsOpenAIServiceTierValue(model, choice.tier),
          )
        : [],
    getEffectiveTier: effectiveOpenAIServiceTier,
    patchServiceTier: (context, tier) =>
      patchOpenAIServiceTier(context.model, context.payload, tier),
  },
];

export const getApplicableTierChoices = (
  model: ModelLike | undefined,
): readonly TierChoice[] =>
  PATCHERS.flatMap((patcher) => patcher.getTierChoices(model));

export const getEffectiveServiceTier = (
  model: ModelLike | undefined,
  provider: ProviderOptionsProvider,
  tier: ServiceTier,
): ServiceTier => {
...
export const patchProviderOptionsRequest = (
  context: RequestPatchContext,
): unknown | undefined => {
  ...
  for (const patcher of PATCHERS) {
    ...
    const patched = patcher.patchServiceTier(
