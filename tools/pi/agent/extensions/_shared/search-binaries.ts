...
const executableNames = (name: string): string[] => {
	...
	const hasExecutableExtension = [".exe", ".cmd", ".bat", ".com"].some((ext) => lower.endsWith(ext));
...
const pathCandidates = (name: string): string[] => {
	...
	return dirs.flatMap((dir) => executableNames(name).map((candidate) => path.join(dir, candidate)));
};

const managedCandidates = (name: string): string[] =>
	executableNames(name).map((candidate) => path.join(getAgentDir(), "bin", candidate));
...
export const resolveSearchBinary = (name: "fd" | "rg"): string | undefined => {
	for (const candidate of [...managedCandidates(name), ...pathCandidates(name)]) {
