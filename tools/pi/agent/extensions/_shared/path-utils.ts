...
export const relativePosixPath = (cwd: string, absolutePath: string): string => {
	const relativeToCwd = path.relative(cwd, absolutePath);
	return toPosixPath(relativeToCwd.length === 0 ? path.basename(absolutePath) : relativeToCwd);
};
