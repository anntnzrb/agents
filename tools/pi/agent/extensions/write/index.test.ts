...
import { lstatSync, linkSync, mkdirSync, readdirSync, readFileSync, readlinkSync, rmSync, symlinkSync, writeFileSync } from "node:fs";
...
mock.module("@mariozechner/pi-coding-agent", () => ({
	...
	createWriteToolDefinition: (cwd: string, options?: { operations?: { mkdir: (dir: string) => Promise<void>; writeFile: (filePath: string, content: string) => Promise<void> } }) => ({
		name: "write",
		async execute(_toolCallId: string, input: { path: string; content: string }) {
			const filePath = input.path.startsWith("/") ? input.path : join(cwd, input.path);
			await options?.operations?.mkdir(dirname(filePath));
			await options?.operations?.writeFile(filePath, input.content);
			return { content: [{ type: "text", text: `Successfully wrote ${input.content.length} bytes to ${input.path}` }], details: undefined };
		},
	}),
...
describe("write execution hardening", () => {
	const makeTempDir = () => join(tmpdir(), `write-exec-${Date.now()}-${Math.random().toString(16).slice(2)}`);

	test("writes atomically and creates parent directories", async () => {
		const dir = makeTempDir();
		const filePath = join(dir, "nested", "file.txt");
		try {
			const result = await __test.executeWrite(dir, { path: filePath, content: "new content" });
	...
	test("expectedHash allows matching current content", async () => {
		...
		try {
			...
			await __test.executeWrite(dir, { path: filePath, content: "new", expectedHash: __test.sha256(Buffer.from("old")) });
	...
	test("expectedHash rejects stale writes and preserves current content", async () => {
		...
		try {
			...
			await expect(
				__test.executeWrite(dir, { path: filePath, content: "stale overwrite", expectedHash: __test.sha256(Buffer.from("old")) }),
	...
	test("atomicWriteFile leaves no temp file on success", async () => {
		...
		try {
			...
			expect(readFileSync(filePath, "utf8")).toBe("new");
			expect(readdirSync(dir).filter((entry) => entry.startsWith(".pi-write-")).length).toBe(0);
		} finally {
			rmSync(dir, { recursive: true, force: true });
		}
	});
