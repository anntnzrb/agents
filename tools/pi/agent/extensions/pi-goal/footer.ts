...
const getCustomEntryType = (entry: unknown): string | undefined =>
  isRecord(entry) && typeof entry["customType"] === "string"
    ? entry["customType"]
    : undefined;

const getCustomEntryData = (entry: unknown): unknown =>
  isRecord(entry) ? entry["data"] : undefined;

const getGoalFromEntry = (entry: unknown): GoalLike | null | undefined => {
  if (
    getEntryType(entry) !== "custom" ||
    getCustomEntryType(entry) !== CUSTOM_TYPE
  )
    return undefined;
...
const buildGoalBadge = (
  ...
  return theme.fg(
    getGoalColor(goal),
    `${getGoalStatusGlyph(goal)}${formatElapsed(goal.timeUsedSeconds)}`,
  );
};
...
export const registerGoalFooterContribution = (): void => {
  registerFooterContribution({
    ...
    render: ({ entries }, theme) =>
      buildGoalBadge(theme, getLatestGoal(entries)),
  });
};
