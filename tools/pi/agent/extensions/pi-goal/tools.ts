...
const getFirstText = (content: unknown): string => {
  ...
  const part = content.find((item) => item && typeof item === "object" && (item as { type?: unknown }).type === "text");
  return typeof (part as { text?: unknown } | undefined)?.text === "string" ? ((part as { text: string }).text) : "";
};

export const registerGoalTools = (pi: ExtensionAPI, runtime: GoalRuntime): void => {
  pi.registerTool({
    name: "update_goal",
    label: "Update Goal",
    description:
    ...
    renderCall(_args, theme, context) {
      return renderReusableText(context.lastComponent, buildUpdateGoalCallText(runtime.goal, theme));
    },
    renderResult(result, _options, theme, context) {
      const rawText = getFirstText((result as { content?: unknown }).content);
      return renderReusableText(context.lastComponent, buildUpdateGoalResultText(rawText, context.isError === true, theme));
    },
    async execute(_toolCallId, input: UpdateGoalInput, _signal, _onUpdate, ctx) {
      if (input.status !== "complete") {
        return { content: [{ type: "text", text: "update_goal can only mark the existing goal complete." }], isError: true };
      }
      if (!runtime.goal) {
