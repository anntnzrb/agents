...
import { getReusableText, joinRenderSegments } from "../_shared/render-utils.js";
...
export const previewObjective = (objective: string, maxChars = 90): string => {
  ...
  const chars = [...compact];
  return chars.length <= maxChars ? compact : `${chars.slice(0, Math.max(0, maxChars - 1)).join("")}…`;
};
...
const shouldShowSnapshotUsage = (kind: GoalEventKind): boolean =>
  kind === "paused" || kind === "resumed" || kind === "complete" || kind === "cleared";

const buildCollapsedGoalEventText = (kind: GoalEventKind, state: GoalState | null, theme: Theme): string => {
  const segments = [
    `${theme.fg("muted", goalEventIcon(kind))} ${theme.fg("toolTitle", theme.bold("goal"))}`,
    theme.fg("muted", goalEventStatus(kind)),
  ];
  if (state && shouldShowSnapshotUsage(kind)) segments.push(theme.fg("muted", goalUsage(state)));
  const header = joinRenderSegments(segments, theme);
  return state ? `${header}\n${objectivePreviewLine(state, theme)}` : header;
};

export const buildGoalEventText = (message: MessageLike, options: RendererOptions, theme: Theme): string => {
...
export const buildUpdateGoalCallText = (state: GoalState | null, theme: Theme): string => {
  const header = joinRenderSegments([`${theme.fg("muted", "✓")} ${theme.fg("toolTitle", theme.bold("goal"))}`, theme.fg("muted", "complete")], theme);
  return state ? `${header}\n${objectivePreviewLine(state, theme)}` : header;
};

export const buildUpdateGoalResultText = (rawText: string, isError: boolean, theme: Theme): string => {
  if (!isError) return "";
...
export type MessageRendererApi = {
  registerMessageRenderer: (
    customType: string,
    renderer: (message: MessageLike, options: RendererOptions, theme: Theme) => Text
  ) => void;
};
...
export const registerGoalRenderer = (pi: MessageRendererApi): void => {
  pi.registerMessageRenderer(
    EVENT_TYPE,
    (message, options, theme) => new Text(buildGoalEventText(message, options, theme), 0, 0)
  );
};

export const renderReusableText = (lastComponent: unknown, value: string): Text => {
