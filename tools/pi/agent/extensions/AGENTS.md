...
- In async tool handlers, UI render callbacks, event listeners, and Promise constructors, avoid synchronous child-process APIs (`execFileSync`, `spawnSync`, `execSync`) because they freeze the TUI/agent event loop.
...
- Cold-path binary resolution (`search-binaries.ts`) may remain sync when cached and not invoked per tool call; document the exception explicitly.
- When a sync interface wraps an async implementation (e.g. `refresh: () => void` calling `async` code), either: (a) swallow rejections with try/catch inside, or (b) change the interface to return `Promise<void>` and await call sites. Do not discard returned Promises silently.
...
- Read-only analysis: do not mutate or validate beyond inspection.
