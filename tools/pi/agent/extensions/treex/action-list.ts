...
class ActionList {
  ...
  render(width: number): string[] {
    ...
    if (visible.length === 0)
      return [
        truncateToWidth(this.theme.fg("muted", "  No entries on path"), width),
      ];

    this.selectedVisibleIndex = Math.max(
      0,
      Math.min(this.selectedVisibleIndex, visible.length - 1),
    );

    const start = Math.max(
      0,
      Math.min(
        this.selectedVisibleIndex - Math.floor(this.maxVisibleLines / 2),
    ...
    lines.push(
      truncateToWidth(
        this.theme.fg(
          "muted",
          `  (${this.selectedVisibleIndex + 1}/${visible.length})`,
        ),
        width,
      ),
    );
  ...
  private jumpGroup(visible: VisibleActionRow[], direction: "up" | "down") {
    ...
    while (
      ...
      visible[last + 1]?.groupId === currentGroup
    )
      last++;
    ...
    while (
      ...
      visible[lastOfNext + 1]?.groupId === nextGroupId
    )
      lastOfNext++;
  ...
  private formatGroupPrefix(
    position: VisibleActionRow["groupPosition"],
  ): string {
    ...
    switch (entry.type) {
      case "message": {
        ...
        if (role === "user") {
          result =
            ...
            normalize(extractTextContent(msg?.content));
        } else if (role === "assistant") {
          const content = normalize(extractTextContent(msg?.content));
          if (content)
            result = this.theme.fg("success", "assistant: ") + content;
          else if (msg?.errorMessage)
            result =
              this.theme.fg("success", "assistant: ") +
              this.theme.fg("error", normalize(msg.errorMessage).slice(0, 120));
          ...
          result = this.theme.fg(
            "dim",
            `[bash]: ${normalize(String(msg?.command ?? ""))}`,
          );
        } else {
      ...
      case "branch_summary":
        result =
          ...
          normalize(String(entry.summary ?? ""));
        break;
      case "compaction":
        result = this.theme.fg(
          "borderAccent",
          `[compaction: ${Math.round(Number(entry.tokensBefore ?? 0) / 1000)}k tokens]`,
        );
        break;
      default:
...
export async function showActionList(
  ...
  return (ctx.ui as any).custom((tui: any, theme: any, _kb: any, done: any) => {
    ...
    container.addChild(
      new Text(theme.fg("accent", theme.bold("Treex Actions")), 1, 0),
    );
    container.addChild(
      new Text(
        theme.fg(
          "muted",
          "Choose what should happen to each group. P = pick, S = summarize, D = drop",
        ),
        1,
        0,
      ),
