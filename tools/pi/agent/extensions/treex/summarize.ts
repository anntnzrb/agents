...
type TreexRewriteContext = ExtensionCommandContext & {
    model?: TreexModel;
    modelRegistry: {
        getApiKeyAndHeaders(model: TreexModel): Promise<
...
function entryFileOps(entry: SessionEntry): FileOps {
    ...
    if (entry.type === "branch_summary" && !entry["fromHook"] && entry["details"] && typeof entry["details"] === "object") {
...
function computeFileLists(entries: SessionEntry[]): { readFiles: string[]; modifiedFiles: string[] } {
...
function summarizeToolCalls(content: unknown): string[] {
    if (!Array.isArray(content)) return [];
    return content
        .filter((block) => block && typeof block === "object" && "type" in block && block.type === "toolCall")
        .map((block: any) => {
...
function serializeEntry(entry: SessionEntry): string {
    if (entry.type === "message") {
        ...
        if (role === "toolResult") {
            const tool = entry.message?.toolName ?? entry.message?.toolCallId ?? "tool";
...
function parseSummaryJson(responseText: string): Map<string, string> {
    ...
    try {
        ...
        throw new Error(`Could not parse summarizer JSON: ${responseText.slice(0, 500)}`);
    }
...
async function summarizeGroups(
    ...
    const result = await ctx.ui.custom<string | null>((tui: any, theme: any, _kb: any, done: any) => {
        const loader = new BorderedLoader(tui, theme, `Treex summarizing with ${model.id ?? "selected model"}...`);
        ...
        run().then(done).catch((error) => done(JSON.stringify({ error: String(error?.message ?? error) })));
        return loader;
    });
...
export async function buildRewrite(
    ...
    const groups = groupSummaryItems(items);
    const summaries = groups.length > 0 ? await summarizeGroups(ctx, items) : new Map<string, string>();
    ...
    for (const item of items) {
        ...
        const { readFiles, modifiedFiles } = computeFileLists(group.items.map((entryItem) => entryItem.entry));
        parts.push({
            kind: "summary",
            priority: "combined",
