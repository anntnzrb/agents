...
const formatLineStats = (stats: LineStats): string =>
...
const formatEditCount = (count: number): string =>
  `${count} ${pluralize(count, "edit")}`;

const getEditSummary = (
  args: EditArgs,
  previous?: EditSummary,
): EditSummary => {
...
const buildCollapsedEditCallText = (
  ...
  const segments = [
    `${theme.fg("muted", "✎")} ${theme.fg("toolTitle", theme.bold("edit"))} ${theme.fg("muted", summary.path)}`,
  ];

  if (summary.count !== undefined && summary.stats) {
    segments.push(
      formatEditCount(summary.count),
      formatColoredLineStats(summary.stats, theme),
    );
  }
      ...
      if (context.isError) {
        text.setText(
          theme.fg(
            "error",
            getFirstTextContent(
              getTextParts((result as RenderResult).content),
            ) || "(no output)",
          ),
        );
        return text;
