...
async function selectSummaryModel(
	...
	if (ctx.model.provider === "anthropic") {
		...
		if (haikuModel) {
			...
			if (auth.ok && auth.apiKey) {
				return auth.headers
					? { model: haikuModel, apiKey: auth.apiKey, headers: auth.headers }
					: { model: haikuModel, apiKey: auth.apiKey };
			}
		}
	}
	...
	return auth.headers
		? { model: ctx.model, apiKey: auth.apiKey, headers: auth.headers }
...
export default function loopExtension(pi: ExtensionAPI): void {
	...
	pi.registerTool({
		...
		promptSnippet: "Stop an active /loop cycle once the breakout condition is met.",
		promptGuidelines: ["Only call signal_loop_success when the current loop breakout condition is explicitly satisfied."],
	...
	pi.registerCommand("loop", {
		...
		handler: async (args, ctx) => {
			...
			const summarizedState: LoopStateData = { ...nextState, loopCount: 0 };
