...
describe("captureBatch", () => {
	...
	test("keeps legacy input/args fallback only as compatibility at capture boundary", () => {
			...
			[
				{
					role: "toolResult",
					toolCallId: "input-id",
					content: [textPart("/tmp")],
					isError: false,
				},
				{
					role: "toolResult",
					toolCallId: "args-id",
					content: [textPart("index.ts")],
					isError: false,
				},
			],
			1,
		...
		expect(batch.toolCalls.map((call) => call.arguments)).toEqual([
			{ command: "pwd" },
			{ pattern: "*.ts" },
		]);
	});
			...
			[
				{
					role: "toolResult",
					toolCallId: "recovery",
					content: [textPart("huge recovered exact output")],
					isError: false,
				},
			],
			1,
...
describe("collect threshold", () => {
	...
	test("collects one large result or large accumulated output", () => {
		expect(
			shouldCollect([batchWithResult("x".repeat(LARGE_SINGLE_RESULT_CHARS))]),
		).toBe(true);
		expect(
			shouldCollect([
				batchWithResult("x".repeat(MIN_PENDING_RESULT_CHARS / 2)),
				batchWithResult("y".repeat(MIN_PENDING_RESULT_CHARS / 2)),
			]),
		).toBe(true);
	});
	...
	test("counts result characters across batches", () => {
		expect(
			totalResultChars([batchWithResult("abc"), batchWithResult("defg")]),
		).toBe(7);
	});
});
...
describe("context compaction", () => {
	test("removes only collected toolResult messages", () => {
		const messages = [
			...
			},
			{
				role: "toolResult",
				toolCallId: "call-1",
				content: [textPart("raw")],
				isError: false,
			},
			{
				role: "toolResult",
				toolCallId: "call-2",
				content: [textPart("keep")],
				isError: false,
			},
	...
	test("restores full context-gc summary text for provider context without changing UI content", () => {
		const messages = [
			{
				role: "custom",
				customType: "context-gc-summary",
				content: "Context GC: compacted 1 tool output(s) · 12k chars",
				details: {
					summaryText: "# Context GC summary\n\nFull model-facing summary",
				},
			},
		];

		expect(restoreSummaryContentForContext(messages)).toEqual([
			{
				role: "custom",
				customType: "context-gc-summary",
				content: "# Context GC summary\n\nFull model-facing summary",
				details: {
					summaryText: "# Context GC summary\n\nFull model-facing summary",
				},
			},
...
describe("index persistence", () => {
	test("stores exact original outputs and reconstructs from custom entries", () => {
		...
		expect((piEntries[0] as { customType: string }).customType).toBe(
			CUSTOM_TYPE_INDEX,
		);

		const restored = new ToolResultIndex();
		restored.reconstructFromSession({
			sessionManager: { getEntries: () => piEntries as never[] },
		});
		expect(restored.lookup(["call-1"])[0]?.resultText).toBe(
			"original exact output",
		);
		expect(formatRecordsForQuery(restored.lookup(["call-1"]))).toContain(
			"original exact output",
		);
	});
});
...
describe("summary serialization", () => {
	test("includes ids and keeps serialized fallback compact while originals remain separate", () => {
		const serialized = serializeBatchesForSummary([
			batchWithResult("z".repeat(5_000)),
		]);
