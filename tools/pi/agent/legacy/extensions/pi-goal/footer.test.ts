...
import {
  ...
  getFooterContributions,
} from "../_shared/footer-contributions.js";
...
describe("pi-goal footer contribution", () => {
  ...
  test("renders goal badges", () => {
    expect(
      __test.buildGoalBadge(theme, { status: "active", timeUsedSeconds: 10 }),
    ).toBe("⚑10s");
    expect(
      __test.buildGoalBadge(theme, { status: "paused", timeUsedSeconds: 90 }),
    ).toBe("‖1m");
    expect(
      __test.buildGoalBadge(theme, { status: "complete", timeUsedSeconds: 90 }),
    ).toBe("✓1m");
  });
  ...
  test("registers optional footer contribution", () => {
    ...
    const contribution = getFooterContributions().find(
      (entry) => entry.id === "pi-goal",
    );
          ...
          entries: [
            {
              type: "custom",
              customType: "pi-goal",
              data: { goal: { status: "active", timeUsedSeconds: 10 } },
            },
          ],
        },
        theme,
