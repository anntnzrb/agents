...
$HostIsWindows = $Platform -match '^Microsoft Windows|^Windows'
$HostIsLinux = $Platform -match '^Linux'
$HostIsMacOS = $Platform -match '^macOS|^Darwin'
...
function Get-CurrentShellExecutable {
    ...
    try {
        $current = Get-Command -Name pwsh -ErrorAction Stop
    ...
    try {
        $fallback = Get-Command -Name powershell.exe -ErrorAction Stop
...
$root = Join-Path ([System.IO.Path]::GetTempPath()) ("omp-pwsh-suite-" + [guid]::NewGuid())
$currentShell = Get-CurrentShellExecutable
...
'@
    ...
    if ($PSVersionTable.PSVersion -ge [Version]'7.0.0') {
        Test-Case 'chain.and-operator' {
            $out = [scriptblock]::Create("(& { 'A' }) && (& { 'B' })").Invoke()
            $joined = ($out -join ',')
        ...
        if ($currentShell) {
            Test-Case 'chain.or-operator' {
                $escapedShell = $currentShell.Replace("'", "''")
                $out = [scriptblock]::Create("(& '$escapedShell' -NoProfile -Command 'exit 1') || (& { 'RECOVER' })").Invoke()
                $joined = ($out -join ',')
            ...
            Skip-Case -Name 'chain.or-operator' -Reason 'Current shell executable not discoverable'
        }
    }
    else {
        Skip-Case -Name 'chain.and-operator' -Reason 'PowerShell chain operators require PowerShell >= 7.0'
        Skip-Case -Name 'chain.or-operator' -Reason 'PowerShell chain operators require PowerShell >= 7.0'
    }
    ...
    if ($PSVersionTable.PSVersion -ge [Version]'7.4.0') {
        if ($currentShell) {
            Test-Case 'native.commandwithargs-escaping' {
                $out = & $currentShell -NoProfile -CommandWithArgs '$args -join "|"' 'a b' 'c"d' "e'f" '$x'
            ...
            Skip-Case -Name 'native.commandwithargs-escaping' -Reason 'Current shell executable not discoverable'
        }
    }
    else {
        Skip-Case -Name 'native.commandwithargs-escaping' -Reason 'CommandWithArgs requires PowerShell >= 7.4'
    }
...
[pscustomobject]@{
    host = [pscustomobject]@{
        ...
        isWindows = [bool]$HostIsWindows
        isLinux = [bool]$HostIsLinux
        isMacOS = [bool]$HostIsMacOS
    }
