...
Invalid committed configuration, malformed local secrets, hook failures, and a failed first managed-tool installation are fatal. A client host can operate without `secrets.local.json`.
