...
interface AutommitFileHandle {
    readonly writeFile: (content: string, encoding?: string) => Promise<void>;
...
interface AutommitStats {
    readonly isDirectory: () => boolean;
    readonly isFile: () => boolean;
    readonly isSymbolicLink: () => boolean;
}

const openFile = (path: string, flags: string, mode?: number): Promise<AutommitFileHandle> =>
    open(path, flags, mode) as unknown as Promise<AutommitFileHandle>;

const lstatEntry = async (path: string): Promise<AutommitStats> =>
    lstat(path) as unknown as AutommitStats;
...
const ensureNoSymlinkPath = async (
    ...
    const root = pathRoot(absolute);
    const baseRoot = pathRoot(base);
    ...
    for (const component of components.length === 0 ? [""] : components) {
        ...
        try {
            const stats = await lstatEntry(current);
...
const ensureCommonDirectory = async (paths: Paths): Promise<void> => {
    ...
    const baseStats = await lstatEntry(paths.commonDir).catch(error => {
...
const ensureAutommitDirectory = async (paths: Paths): Promise<void> => {
    ...
    const directoryStats = await lstatEntry(paths.directory).catch(error => {
...
const readBoundedText = async (target: string, maxBytes: number): Promise<string> => {
    let handle: AutommitFileHandle | undefined;
    try {
...
const existingRegularFile = async (target: string, kind: string, commonDir: string): Promise<boolean> => {
    ...
    const stats = await lstatEntry(target).catch(error => {
...
/**
 * Consumes a completed receipt (state "committed") by removing it, so a
 * follow-up run does not try to publish the same commit twice. Prepared
 * receipts are returned unchanged for recovery.
 */
