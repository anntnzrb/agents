...
import {
  loadSettings,
  saveSettings,
  type ProviderOptionsSettings,
} from "./settings.js";

const STATUS_KEY = "provider-patcher";

const formatStatus = (tier: ServiceTier): string =>
  `tier: openai=${tierLabel(tier)}`;
...
const findChoiceByText = (
  choices: readonly TierChoice[],
  current: ServiceTier,
  text: string,
...
export default function (pi: ExtensionAPI) {
  ...
  pi.on("before_provider_request", async (event, ctx) => {
    ...
    return patchProviderOptionsRequest({
      model: ctx.model as Parameters<
        typeof patchProviderOptionsRequest
      >[0]["model"],
  ...
  pi.registerCommand("tier", {
    description:
      "Choose the request service tier for the currently selected model",
    handler: async (_args: string, ctx: ExtensionCommandContext) => {
      await ensureLoaded();

      const choices = getApplicableTierChoices(
        ctx.model as Parameters<typeof getApplicableTierChoices>[0],
      );
      if (choices.length === 0) {
        ctx.ui.notify(
          "No service-tier patch applies to the currently selected model.",
          "info",
        );
        return;
      }
      ...
      if (!ctx.hasUI) {
        ctx.ui.notify(
          `${formatStatus(effectiveTier(ctx.model as Parameters<typeof getEffectiveServiceTier>[0]))}. Run /tier in interactive mode to choose.`,
          "info",
        );
        return;
      }
      ...
      ).select("Choose service tier for current model", labels);
      if (!selected) return;
