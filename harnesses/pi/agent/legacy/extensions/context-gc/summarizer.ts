...
const compactResult = (text: string): string => {
  ...
  if (omitted > 0)
    preview.push(
      `…[${omitted} lines omitted; exact original recoverable by toolCallId]`,
    );
  if (preview.length === 0 && text.length > 0)
...
export function summarizeBatches(
  ...
  const sections = batches.map((batch) => {
    ...
    return [
      `## Turn ${batch.turnIndex}`,
      batch.assistantText
        ? `Assistant text: ${batch.assistantText}`
        : undefined,
      ...calls,
    ]
  ...
  return {
    text: [
      `# Context GC summary`,
      `Compacted ${details.toolCallIds.length} tool output(s), ${details.totalResultChars} raw chars.`,
      ...sections,
      `Exact originals: ${ids}`,
      `Use \`context_tree_query\` with those IDs if exact output is needed.`,
    ].join("\n\n"),
  };
}
