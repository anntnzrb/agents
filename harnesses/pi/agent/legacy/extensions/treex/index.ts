...
async function navigateToRewrittenLeaf(
  ...
  if (isEditableNavigationTarget(rewrittenLeaf)) {
    ...
    if (result.cancelled)
      throw new Error("navigateTree cancelled unexpectedly");
    return;
  }
...
async function chooseAncestorMode(
  ctx: TreexContext,
): Promise<"navigate" | "rewrite" | null> {
...
async function runTreexCommand(
  pi: ExtensionAPI,
  rawCtx: ExtensionCommandContext,
): Promise<void> {
  ...
  if (segment.length === 0 || actionItems.length === 0) {
    ctx.ui.notify(
      "Could not compute treex path with any context-bearing entries",
      "error",
    );
    return;
  }
...
export default function (pi: ExtensionAPI): void {
  pi.registerCommand("treex", {
    description:
      "Extended tree navigation with optional interactive history rewrite",
