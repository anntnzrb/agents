...
const buildHeader = (details: ToolDetails): string => {
  ...
  if (details.results.length > 1)
    segments.push(`${details.results.length} tasks`);
...
const buildAggregateHeader = (details: ToolDetails): string => {
  const completed = details.results.filter(
    (result) => getChildRunStatusLabel(result) === "completed",
  ).length;
  const aborted = details.results.filter(
    (result) => getChildRunStatusLabel(result) === "aborted",
  ).length;
  const failed = details.results.filter(
    (result) => getChildRunStatusLabel(result) === "failed",
  ).length;
  const durationMs = Math.max(
    0,
    ...details.results.map((result) => result.durationMs),
  );
  const toolCalls = details.results.reduce(
...
const deriveRenderMode = (
  args: RenderCallArgs,
  taskCount: number,
): "worker" | "explorer" | undefined =>
  args.mode ??
  (taskCount === 1 ? "worker" : taskCount > 1 ? "explorer" : undefined);
  ...
  if (!options.expanded) {
    const lines = details.results.map((task) => {
      ...
      const detail = statusDetail
        ? theme.fg(getResultStatusColor(task), ` — ${statusDetail}`)
        : "";
