...
describe("edit compact helpers", () => {
  ...
  test("extracts only structured edit arrays", () => {
    expect(
      __test.getRenderableEdits({ edits: [{ oldText: "a", newText: "b" }] }),
    ).toEqual([{ oldText: "a", newText: "b" }]);
    expect(
      __test.getRenderableEdits({ edits: '[{"oldText":"a","newText":"b"}]' }),
    ).toBeUndefined();
    expect(
      __test.getRenderableEdits({ oldText: "a", newText: "b" } as never),
    ).toBeUndefined();
    expect(
      __test.getRenderableEdits({ edits: [{ oldText: "a" }] }),
    ).toBeUndefined();
  });
  ...
  test("aggregates edit line stats", () => {
    ...
    expect(__test.formatColoredLineStats(stats, passthroughTheme)).toBe(
      "+3/-3",
    );
    expect(__test.formatColoredLineStats(stats, tokenTheme)).toBe(
      "<toolDiffAdded>+3</toolDiffAdded>/<toolDiffRemoved>-3</toolDiffRemoved>",
    );
  });
...
describe("edit compact rendering", () => {
  ...
  test("summary preserves previous count and delta when args lose edit payload", () => {
    const first = __test.getEditSummary({
      path: "src/foo.ts",
      edits: [{ oldText: "a", newText: "a\nb" }],
    });
    const second = __test.getEditSummary({ path: "src/foo.ts" }, first);
    expect(second).toEqual(first);
    expect(__test.buildCollapsedEditCallText(second, passthroughTheme)).toBe(
      "✎ edit src/foo.ts · 1 edit · +2/-1",
    );
  });
    ...
    const result = registered.renderResult(
      {
        ...
        details: { diff: "diff", firstChangedLine: 37 },
      },
