...
const appendCommonRipgrepArgs = (
  params: Parameters<typeof buildRipgrepCommonArgs>[0] & { args: string[] },
): void => {
...
const resolveRipgrepPath = (
  ...
  const absolutePath = path.isAbsolute(filePath)
    ? filePath
    : path.resolve(rootAbsolute, filePath);
  return {
    absolutePath,
...
export const runRipgrepLineModeEffect = Effect.fn("runRipgrepLineMode")(<T>(
  ...
  return runLineStreamingProcessEffect<T>({
    ...
    skipEmptyLines: true,
    missingBinaryMessage: toolMissingMessage(
      "rg",
      "ripgrep (e.g. `brew install ripgrep`)",
    ),
...
export const runRipgrepCountsEffect = Effect.fn("runRipgrepCounts")((
  ...
  runRipgrepLineModeEffect<CountHit>({
    ...
    parseLine: (line, cwd, rootAbsolute) => {
      ...
      return {
        ...resolveRipgrepPath(cwd, rootAbsolute, line.slice(0, separator)),
        count,
      };
    },
  }),
...
export const runRipgrepEffect = Effect.fn("runRipgrep")((
  ...
  return runLineStreamingProcessEffect<RawMatch>({
    ...
    parseLine: (line) => {
      ...
      const { absolutePath, displayPath } = resolveRipgrepPath(
        cwd,
        rootAbsolute,
        filePath,
      );
      const cleanedLine = (lineText ?? "")
        .replace(/\r\n/g, "\n")
        .replace(/\r/g, "")
        .replace(/\n$/, "");
      return {
