...
const GREP_PROMPT_SNIPPET =
  "Search file contents: output modes, pagination, type filters, ignored controls";
...
const PARAM_DESCRIPTIONS = {
  ...
  pcre2:
    "Enable ripgrep PCRE2 regex engine for look-around/backreferences (default: false)",
...
const grepSchema = Type.Object({
  ...
  paths: Type.Optional(
    Type.Array(Type.String(), {
      description: PARAM_DESCRIPTIONS.paths,
      default: ["."],
    }),
  ),
  ...
  ignoreCase: Type.Optional(
    Type.Boolean({
      description: PARAM_DESCRIPTIONS.ignoreCase,
      default: false,
    }),
  ),
  literal: Type.Optional(
    Type.Boolean({ description: PARAM_DESCRIPTIONS.literal, default: false }),
  ),
  context: Type.Optional(
    Type.Number({ description: PARAM_DESCRIPTIONS.context, default: 0 }),
  ),
  outputMode: Type.Optional(
    Type.String({
      description: PARAM_DESCRIPTIONS.outputMode,
      default: "content",
    }),
  ),
  ignored: Type.Optional(
    Type.Boolean({ description: PARAM_DESCRIPTIONS.ignored, default: false }),
  ),
  pcre2: Type.Optional(
    Type.Boolean({ description: PARAM_DESCRIPTIONS.pcre2, default: false }),
  ),
  offset: Type.Optional(
    Type.Number({ description: PARAM_DESCRIPTIONS.offset, default: 0 }),
  ),
  limit: Type.Optional(
    Type.Number({
  ...
  timeoutMs: Type.Optional(
    Type.Number({
      description: PARAM_DESCRIPTIONS.timeoutMs,
...
const formatOutputModeResult = async (
  ...
  if (outputMode === "files_with_matches") {
    ...
    return {
      rawOutput: files.join("\n"),
      outputLineCount: files.length,
      linesTruncated: false,
    };
  }
  if (outputMode === "count") {
    return {
      rawOutput: String(matches.length),
      outputLineCount: 1,
      linesTruncated: false,
    };
  }
...
export default function grepExtension(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    renderResult(result, _options, theme, context) {
      ...
      if (context.isError) {
        text.setText(
          theme.fg("error", rawText.length > 0 ? rawText : "grep failed"),
        );
        return text;
      }
      text.setText(
        buildCollapsedResultText(
          rawText,
          result.details as GrepRenderDetails | undefined,
          theme,
        ),
      );
      return text;
    ...
    async execute(toolCallId, input: GrepInput, signal, onUpdate, ctx) {
      ...
      const internalProbeLimit = Math.min(
        Math.max(requestedWindow * 5, requestedWindow),
        MAX_INTERNAL_PROBE,
      );
      const cwd = ctx.cwd;
      ...
      if (outputMode === "files_with_matches") {
        ...
        for (const root of roots) {
          ...
          if (remainingTimeoutMs <= 0) {
            throw new Error(
              `grep timed out after ${Math.max(1, Math.round(timeoutMs / 1000))}s`,
            );
          }
        ...
        const orderedFiles = [...collectedFiles].sort((a, b) =>
          a.displayPath.localeCompare(b.displayPath),
        );
        const pagedFiles = orderedFiles.slice(
          effectiveOffset,
          effectiveOffset + effectiveLimit + 1,
        );
        const hasMoreFiles = pagedFiles.length > effectiveLimit;
        const selectedFiles = hasMoreFiles
          ? pagedFiles.slice(0, effectiveLimit)
          : pagedFiles;
        ...
        const details: GrepRenderDetails = {
          ...
          outputLineCount: selectedFiles.length,
        };
        if (hasMoreFiles) {
          notices.push(
            `${effectiveLimit} results limit reached. Use offset=${effectiveOffset + effectiveLimit} for next page`,
          );
      ...
      if (outputMode === "count") {
        ...
        const orderedCounts = [...collectedCounts].sort((a, b) =>
          a.displayPath.localeCompare(b.displayPath),
        );
        const pagedCounts = orderedCounts.slice(
          effectiveOffset,
          effectiveOffset + effectiveLimit + 1,
        );
        const hasMoreCounts = pagedCounts.length > effectiveLimit;
        const selectedCounts = hasMoreCounts
          ? pagedCounts.slice(0, effectiveLimit)
          : pagedCounts;
        ...
        const details: GrepRenderDetails = {
          ...
          fileCount: selectedCounts.length,
          outputLineCount: selectedCounts.length + 2,
        };
        if (hasMoreCounts) {
          notices.push(
            `${effectiveLimit} results limit reached. Use offset=${effectiveOffset + effectiveLimit} for next page`,
      ...
      const contextLines =
        outputMode === "content" ? (requestedContext ?? 0) : 0;
      const { rawOutput, outputLineCount, linesTruncated } =
        await formatOutputModeResult(selected, outputMode, contextLines);
      const truncation = truncateHead(rawOutput, {
        maxLines: Number.MAX_SAFE_INTEGER,
        maxBytes: DEFAULT_MAX_BYTES,
      });
      let output = truncation.content;
      const notices: string[] = [];
      const fileCount = new Set(selected.map((match) => match.displayPath))
        .size;
      const details: GrepRenderDetails = {
        outputMode,
      ...
      if (hasMore) {
        notices.push(
          `${effectiveLimit} matches limit reached. Use offset=${effectiveOffset + effectiveLimit} for next page`,
        );
        details.matchLimitReached = effectiveLimit;
      }
      if (truncation.truncated) {
