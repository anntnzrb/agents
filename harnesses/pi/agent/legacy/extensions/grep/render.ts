...
const stripNoticeSuffix = (text: string): string =>
  text.replace(/\n\n\[[^\n]+\]$/, "");
...
const summarizeOutput = (
  output: string,
): { matchCount: number; fileCount: number; lineCount: number } => {
  let matchCount = 0;
...
export const formatGrepCall = (
  ...
  if (input.outputMode && input.outputMode !== "content")
    flags.push(theme.fg("accent", formatOutputMode(input.outputMode)));
  ...
  if (input.context !== undefined)
    flags.push(theme.fg("muted", `ctx ${input.context}`));
  if (input.ignored) flags.push(theme.fg("warning", RENDER_LABELS.ignoredOn));
  if ((input.offset ?? 0) > 0)
    flags.push(theme.fg("muted", `${RENDER_LABELS.offset}:${input.offset}`));
  ...
  return joinRenderSegments(
    [
      `${theme.fg("muted", "⌕")} ${theme.fg("toolTitle", theme.bold("grep"))} ${theme.fg("muted", scope)}`,
      ...flags,
    ],
    theme,
  );
...
export const buildCollapsedResultText = (
  ...
  if (
    ...
    lines.push(
      `${prefix} ${matchCount} ${pluralize(matchCount, "match", "matches")} in ${fileCount} ${pluralize(fileCount, "file")}`,
    );
  } else {
    const fallback = summarizeOutput(body);
    if (fallback.matchCount > 0) {
      lines.push(
        `↳ ${fallback.matchCount} ${pluralize(fallback.matchCount, "match", "matches")} in ${fallback.fileCount} ${pluralize(fallback.fileCount, "file")}`,
      );
    } else {
