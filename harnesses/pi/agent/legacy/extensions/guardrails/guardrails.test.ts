    ...
    rules: [
      ...
      },
      {
        ...
        match: {
          ...
          patterns: [
            "^(?:conda|hatch|mamba|pdm|pipenv|poetry|rye|virtualenv)(?:\\.exe)?$",
          ],
        },
        action: {
          type: "block",
...
test("loadConfig fails when requiresBinding references unknown skill binding", () => {
  ...
  assert.match(
    result.reason,
    /requiresBinding references unknown skillBindings key/,
  );
});
...
test("blocks direct python heredoc stdin invocation", () => {
  assert.equal(
    reasonForCommand("python3 - <<'PY'\nprint(1)\nPY", pythonConfig),
    "use uv",
  );
  assert.equal(
    reasonForCommand("python - <<PY\nprint(1)\nPY", pythonConfig),
    "use uv",
  );
});
...
test("blocks versioned and alternate python executables", () => {
  assert.equal(
    reasonForCommand('.venv/bin/python3.12 -c "print(1)"', pythonConfig),
    "use uv",
  );
  assert.equal(
    reasonForCommand('python3.12.exe -c "print(1)"', pythonConfig),
    "use uv",
  );
  assert.equal(
    reasonForCommand("pythonw.exe script.py", pythonConfig),
    "use uv",
  );
  assert.equal(reasonForCommand("py -3 script.py", pythonConfig), "use uv");
  assert.equal(
    reasonForCommand("py.exe -3.12 script.py", pythonConfig),
    "use uv",
  );
...
test("blocks python packaging and environment tools", () => {
  assert.equal(
    reasonForCommand("pip3.12 install rich", pythonConfig),
    "use uv",
  );
  assert.equal(
    reasonForCommand("pip.exe install rich", pythonConfig),
    "use uv",
  );
...
test("blocks env wrapped python invocation", () => {
  assert.equal(
    reasonForCommand(
      "/usr/bin/env PYTHONPATH=. python script.py",
      pythonConfig,
    ),
    "use uv",
  );
  assert.equal(
    reasonForCommand("/usr/bin/env -S py -3 script.py", pythonConfig),
    "use uv",
  );
});
...
test("blocks python module package tooling", () => {
  assert.equal(
    reasonForCommand("uv run python -m pip install rich", pythonConfig),
    "use uv",
  );
});
...
test("blocks sudo wrapped python invocation", () => {
  assert.equal(
    reasonForCommand("sudo -u annt python script.py", pythonConfig),
    "use uv",
  );
});
...
test("blocks shell -c nested python invocation", () => {
  assert.equal(
    reasonForCommand("bash -lc 'python -m pytest'", pythonConfig),
    "use uv",
  );
});
...
test("blocks pwsh -Command nested python invocation", () => {
  ...
  assert.equal(
    reasonForCommand('pwsh.exe -Command "python -m pytest"', pythonConfig),
    "use uv",
  );
  assert.equal(
    reasonForCommand('powershell -Command "python -m pytest"', pythonConfig),
    "use uv",
  );
  assert.equal(
    reasonForCommand(
      'powershell.exe -Command "python -m pytest"',
      pythonConfig,
    ),
    "use uv",
  );
...
test("does not treat quoted heredoc markers as heredocs", () => {
  assert.equal(
    reasonForCommand('echo "<<PY"\npython script.py', pythonConfig),
    "use uv",
  );
});
...
test("warns for shell content-search commands", () => {
  assert.equal(
    reasonForCommand("rg TODO", searchConfig),
    "prefer native grep tool",
  );
  assert.equal(
    reasonForCommand("rg TODO src/main.ts", searchConfig),
    "prefer native grep tool",
  );
  assert.equal(
    reasonForCommand("rg TODO src --glob '*.ts'", searchConfig),
    "prefer native grep tool",
  );
  assert.equal(
    reasonForCommand("rg --hidden --no-ignore TODO src", searchConfig),
    "prefer native grep tool",
  );
  assert.equal(
    reasonForCommand("git grep TODO", searchConfig),
    "prefer native grep tool",
  );
  assert.equal(
    reasonForCommand("git grep TODO src/", searchConfig),
    "prefer native grep tool",
  );
  assert.equal(
    reasonForCommand("grep -R TODO .", searchConfig),
    "prefer native grep tool",
  );
  assert.equal(
    reasonForCommand(
...
test("does not warn when grep filters piped command output", () => {
  assert.equal(
    reasonForCommand(
      "ps -axo pid,command | grep -E 'pi-coding-agent|bun' | grep -v grep",
      searchConfig,
    ),
    null,
  );
  assert.equal(
    reasonForCommand(
      "curl -sS https://example.com | grep -o 'content:\"[^\"]*' | head -1",
      searchConfig,
    ),
    null,
  );
  assert.equal(
    reasonForCommand(
      "jq -r '.models[] | .name' data.json | grep -Ei 'glm|kimi|qwen'",
      searchConfig,
    ),
    null,
  );
  assert.equal(
    reasonForCommand(
      "ioreg -r -e AppleSmartBattery | grep -E '(CurrentCapacity|MaxCapacity)'",
      searchConfig,
    ),
    null,
  );
  assert.equal(
    reasonForCommand(
      "vercel projects ls | grep -i 'odoo-meta-connector' || true",
      searchConfig,
    ),
    null,
  );
...
test("still warns when piped grep is file or repository search", () => {
  assert.equal(
    reasonForCommand("cat src/main.ts | grep TODO", searchConfig),
    null,
  );
  assert.equal(
    reasonForCommand("find . -type f | xargs grep TODO", searchConfig),
    "prefer native find tool",
  );
  assert.equal(
    reasonForCommand(
      "printf '%s\\n' src/main.ts | xargs grep TODO",
      searchConfig,
    ),
    null,
  );
  assert.equal(
    reasonForCommand("grep TODO src/main.ts | head -1", searchConfig),
    "prefer native grep tool",
  );
  assert.equal(
    reasonForCommand("rg TODO src | head -1", searchConfig),
    "prefer native grep tool",
  );
});
...
test("agent warning hints name native replacements and shell executables", () => {
  const tools = [
    ...
    },
    {
      name: "find",
      parameters: {
        properties: { pattern: {}, paths: {}, kind: {}, ignored: {} },
      },
    },
  ];

  const grepHint = agentHintForWarning(
    "Use native `grep` tool for repo search.",
    tools,
  );
  ...
  assert.match(
    grepHint,
    /grep\(\{ pattern, paths, outputMode, ignored, timeoutMs \}\)/,
  );

  const findHint = agentHintForWarning(
    "Use native `find` tool for file lookup.",
    tools,
  );
...
test("block hints are generic and skill-driven", () => {
  const hint = agentHintForBlock({
    message:
      "Python env/package tooling is disabled. Use uv-based Python workflow.",
    requiresSkill: "python",
    requiredWorkflow:
      "Use uv workflow (`uv run`, `uv add`, `uv run --with`) and avoid raw python/pip/env tooling.",
  });
  assert.match(hint, /Required skill `python` has been loaded into context/);
  assert.match(hint, /uv run/);
  assert.equal(
    agentHintForBlock("Reading .env files is blocked by guardrails."),
    "Reading .env files is blocked by guardrails.",
  );
});

test("warns for broad shell file-discovery commands", () => {
  assert.equal(
    reasonForCommand("rg --files", searchConfig),
    "prefer native find tool",
  );
  assert.equal(
    reasonForCommand("ripgrep --files src", searchConfig),
    "prefer native find tool",
  );
  assert.equal(
    reasonForCommand("rg --files | nl -ba", searchConfig),
    "prefer native find tool",
  );
  assert.equal(
    reasonForCommand("git ls-files", searchConfig),
    "prefer native find tool",
  );
  assert.equal(
    reasonForCommand("git ls-files '*.ts'", searchConfig),
    "prefer native find tool",
  );
  assert.equal(
    reasonForCommand("fd '*.ts'", searchConfig),
    "prefer native find tool",
  );
  assert.equal(
    reasonForCommand("fd-find '*.ts' .", searchConfig),
    "prefer native find tool",
  );
  assert.equal(
    reasonForCommand("gfind . -name '*.ts'", searchConfig),
    "prefer native find tool",
  );
  assert.equal(
    reasonForCommand("find . -name '*.ts'", searchConfig),
    "prefer native find tool",
  );
  assert.equal(
    reasonForCommand("find src -name '*.ts'", searchConfig),
    "prefer native find tool",
  );
  assert.equal(
    reasonForCommand("find src -maxdepth 2 -name '*.ts'", searchConfig),
    "prefer native find tool",
  );
  assert.equal(
    reasonForCommand("find / -name '*.ts'", searchConfig),
  ...
  assert.equal(
    reasonForCommand("find . -name '*.ts' | head", searchConfig),
    "prefer native find tool",
  );
  assert.equal(
    reasonForCommand("echo x | find . -name '*.ts'", searchConfig),
    "prefer native find tool",
  );
  assert.equal(
    reasonForCommand(
  ...
  assert.equal(
    reasonForCommand("env FOO=1 find . -name '*.ts'", searchConfig),
    "prefer native find tool",
  );
  assert.equal(
    reasonForCommand("command find . -name '*.ts'", searchConfig),
    "prefer native find tool",
  );
  assert.equal(
    reasonForCommand("sudo find . -name '*.ts'", searchConfig),
    "prefer native find tool",
  );
  assert.equal(
    reasonForCommand("locate js", searchConfig),
    "prefer native find tool",
  );
  assert.equal(
    reasonForCommand("bash -lc 'find . -name \"*.ts\"'", searchConfig),
    "prefer native find tool",
  );
  assert.equal(
    reasonForCommand("sh -c 'find src -name \"*.ts\"'", searchConfig),
    "prefer native find tool",
  );
...
test("does not warn for harmless mentions of search tool names", () => {
  ...
  assert.equal(
    reasonForCommand("printf 'use find and grep tools'", searchConfig),
    null,
  );
  assert.equal(reasonForCommand("command -v rg", searchConfig), null);
  assert.equal(reasonForCommand("where git", searchConfig), null);
  assert.equal(
    reasonForCommand("pwsh -NoProfile -Command 'gci -File'", searchConfig),
    null,
  );
});
...
test("regex rules apply inside nested shell wrappers", () => {
  ...
  assert.equal(
    reasonForCommand("bash -lc 'python -m pip install rich'", config),
    "use uv add or uv run --with",
  );
});

test("blocks protected reads", () => {
  assert.equal(
    reasonForPath("/tmp/project/.env", "read", pythonConfig),
    "no env reads",
  );
});
...
test("does not overblock .env.example", () => {
  assert.equal(
    reasonForPath("/tmp/project/.env.example", "read", pythonConfig),
    null,
  );
});
...
test("blocks protected directory roots and children", () => {
  ...
  assert.equal(
    reasonForPath("/tmp/project/.git", "read", config),
    "no git reads",
  );
  assert.equal(
    reasonForPath("/tmp/project/.git/config", "read", config),
    "no git reads",
  );
});
...
test("allows node_modules reads while still supporting node_modules write blocks", () => {
  ...
  assert.equal(
    reasonForPath("/tmp/project/node_modules", "read", config),
    null,
  );
  assert.equal(
    reasonForPath("/tmp/project/node_modules/react/index.js", "read", config),
    null,
  );
  assert.equal(
    reasonForPath("/tmp/project/node_modules", "write", config),
    "no node_modules writes",
  );
  assert.equal(
    reasonForPath("/tmp/project/node_modules/react/index.js", "edit", config),
    "no node_modules writes",
  );
});
