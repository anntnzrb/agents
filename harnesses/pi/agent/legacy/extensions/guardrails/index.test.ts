...
describe("guardrails config cache", () => {
  test("adapts canonical and namespaced shell block events", async () => {
    const { handler, ctx, messages } = setupGuardrailsHandler({
      ...
      agentBash: {
        rules: [
          ...
          {
            match: { type: "executable", names: ["pip"] },
            action: {
              type: "block",
              message: "no pip",
              requiresBinding: "python-tooling",
          ...
          {
            match: { type: "executable", names: ["poetry"] },
            action: {
              type: "block",
              message: "no poetry",
              requiresBinding: "python-tooling",
    ...
    for (const [toolName, command, expected] of [
      [
        "bash",
        "python3 - <<'PY'\nprint(1)\nPY",
        "Required skill `python` has been loaded into context",
      ],
      [
        "functions.bash",
        "pip install rich",
    ...
    expect(messages[0]?.content).toContain(
      '<required_skill_load name="python"',
    );
  });
        ...
        rules: [
          {
            ...
            action: { type: "warn", message: "use grep tool" },
          },
          {
            match: { type: "executable", names: ["fd", "find"] },
            action: { type: "warn", message: "use find tool" },
          },
        ],
      },
      protectedPaths: { rules: [] },
        ...
        rules: [
          {
            pattern: ".env",
            tools: ["read"],
            action: { type: "block", message: "no env reads" },
          },
          {
            pattern: ".env",
            tools: ["write", "edit"],
            action: { type: "block", message: "no env writes" },
          },
        ],
      },
    });
  ...
  test("keeps UI warnings terse while sending fuller agent hints", () => {
    expect(
      __test.agentHintForWarning("Use native `grep` tool for repo search."),
    ).toContain("grep({ pattern, paths");
    expect(
      __test.agentHintForWarning("Use native `find` tool for file lookup."),
    ).toContain("find({ pattern, paths");
  });
});
