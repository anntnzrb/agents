...
describe("grep compact rendering", () => {
  test("call is naked single-line telemetry", () => {
    const text = __test.formatGrepCall(
      { pattern: "TODO", paths: ["src"], type: "ts", literal: true },
      passthroughTheme,
    );
  ...
  test("call spells out offset and limit", () => {
    const text = __test.formatGrepCall(
      { pattern: "TODO", paths: ["src"], offset: 3, limit: 3 },
      passthroughTheme,
    );
  ...
  test("summarizes files output mode as files", () => {
    const text = __test.buildCollapsedResultText(
      "a.ts\nb.ts",
      { outputMode: "files_with_matches", fileCount: 2 },
      passthroughTheme,
    );
