...
const pwshSchema = Type.Object({
  ...
  timeout: Type.Optional(
    Type.Number({ description: "Timeout seconds (optional; no default)" }),
  ),
});
...
const UNIX_PWSH_FALLBACK_PATHS = [
  "/usr/local/bin/pwsh",
...
const withPathPrepended = (baseEnv: NodeJS.ProcessEnv): NodeJS.ProcessEnv => {
  ...
  const pathEntries = [
    ...new Set(
      [ownPath, inheritedPath].flatMap((value) =>
...
const resolvePwshShellConfig = (): PwshShellConfig => {
  if (process.platform === "win32") {
    ...
    throw new Error(
      "PowerShell not found. Install pwsh or ensure powershell.exe is available on PATH.",
    );
  }
  ...
  throw new Error(
    "pwsh not found on PATH. Install PowerShell from https://github.com/PowerShell/PowerShell",
  );
};
  ...
  return {
    exec: (command, cwd, { onData, signal, timeout, env }) => {
      const effect = Effect.callback<{ exitCode: number }, PwshExecutionError>((resume) => {
        ...
        const shellConfig =
          cachedShellConfig ?? (cachedShellConfig = resolvePwshShellConfig());
        const shellCommand = withUtf8Prefix(
          command,
          shellConfig.prependUtf8Prefix,
        );

        const child = spawn(
          shellConfig.shellPath,
          [...shellConfig.args, shellCommand],
          {
            cwd,
...
const enforceWindowsToolPolicy = (pi: ExtensionAPI): void => {
  ...
  const nextTools = withoutBash.includes("pwsh")
    ? withoutBash
    : [...withoutBash, "pwsh"];
  const unchanged =
