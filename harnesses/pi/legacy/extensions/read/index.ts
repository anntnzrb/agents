...
const buildReadCallText = (args: ReadArgs, theme: RenderTheme): string => {
  ...
  const segments = [
    `${theme.fg("muted", "◎")} ${theme.fg("toolTitle", theme.bold("read"))} ${theme.fg("muted", rawPath)}`,
  ];
...
export default function readExtension(pi: ExtensionAPI): void {
  ...
  pi.registerTool({
    ...
    renderResult(result, _options, theme, context) {
      ...
      text.setText(
        buildReadResultText((result.details ?? {}) as ReadDetails, theme),
      );
      return text;
    },
  });
