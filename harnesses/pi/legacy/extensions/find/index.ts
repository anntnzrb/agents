...
const FIND_PROMPT_SNIPPET =
  "Find files/directories by glob: multipath, kind, hidden, ignored controls";
...
const findSchema = Type.Object({
  ...
  hidden: Type.Optional(
    Type.Boolean({ description: PARAM_DESCRIPTIONS.hidden, default: true }),
  ),
  kind: Type.Optional(
    Type.String({ description: PARAM_DESCRIPTIONS.kind, default: "file" }),
  ),
...
const formatFindCall = (input: FindInput, theme: RenderTheme): string => {
  ...
  const scope =
    pathRoots.length > 0 ? `paths:${summarizeList(displayRoots)}` : ".";
  const flags: string[] = [theme.fg("accent", pattern)];
  if (input.kind && input.kind !== "file")
    flags.push(theme.fg("accent", input.kind));
  if (input.hidden === false)
    flags.push(theme.fg("muted", RENDER_LABELS.visible));
  if (input.ignored === true)
    flags.push(theme.fg("warning", RENDER_LABELS.ignoredOn));
  if (input.limit !== undefined)
    flags.push(theme.fg("muted", `${RENDER_LABELS.limit}:${input.limit}`));
  if (input.timeoutMs !== undefined)
    flags.push(theme.fg("muted", `${input.timeoutMs}ms`));

  return joinRenderSegments(
    [
      `${theme.fg("muted", "◇")} ${theme.fg("toolTitle", theme.bold("find"))} ${theme.fg("muted", scope)}`,
      ...flags,
    ],
    theme,
  );
...
const getCollapsedSummary = (
  ...
  if (details.truncation?.truncated)
    segments.push(
      theme.fg("warning", `${formatSize(DEFAULT_MAX_BYTES)} output limit`),
    );
...
const runFd = async (params: {
  ...
  return await runLineStreamingProcess<string>({
    ...
    timeoutErrorMessage: (ms) =>
      `find timed out after ${Math.max(1, Math.round(ms / 1000))}s`,
    parseLine: (line) => line,
  });
};
  ...
  pi.registerTool({
    ...
    renderResult(result, _options, theme, context) {
      ...
      const summary = getCollapsedSummary(
        rawText,
        (result.details ?? {}) as FindRenderDetails,
        theme,
      );
    ...
    async execute(toolCallId, input: FindInput, signal, onUpdate, ctx) {
      ...
      for (const root of roots) {
        ...
        if (remainingTimeoutMs <= 0) {
          throw new Error(
            `find timed out after ${Math.max(1, Math.round(timeoutMs / 1000))}s`,
          );
        }
        ...
        for (const matchPath of matches) {
          const resolved = path.isAbsolute(matchPath)
            ? matchPath
            : path.resolve(absoluteRoot, matchPath);
