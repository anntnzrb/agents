...
test("buildFdArgs applies kind and ignore controls", () => {
  ...
  assert.deepEqual(
    fileArgs.slice(fileArgs.indexOf("--type"), fileArgs.indexOf("--type") + 2),
    ["--type", "file"],
  );
  assert.equal(fileArgs.includes("--no-ignore"), false);

  const directoryArgs = buildFdArgs(
    "src*",
    "/repo",
    true,
    5,
  ...
  assert.deepEqual(
    directoryArgs.slice(
      directoryArgs.indexOf("--type"),
      directoryArgs.indexOf("--type") + 2,
    ),
    ["--type", "directory"],
  );
