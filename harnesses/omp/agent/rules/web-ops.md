...
- For an ordinary known page, try `read` first. For open-web discovery in OMP, load `omp-search` and use its documented `uv run --script <skill-dir>/scripts/cli.py` entrypoint through `bash`.
